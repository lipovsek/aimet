...
class SvdAcceptanceTests(unittest.TestCase):
    def test_spatial_svd_compress_manual(self):
        ...
        params = aimet_torch.defs.SpatialSvdParameters(
            aimet_torch.defs.SpatialSvdParameters.Mode.manual,
            manual_params,
            multiplicity=8,
        )
        ...
        self.assertTrue(
            isinstance(compressed_model.layer1[0].conv1, torch.nn.Sequential)
        )
        ...
        self.assertTrue(
            isinstance(compressed_model.layer2[1].conv2, torch.nn.Sequential)
        )
    ...
    def test_spatial_svd_compress_auto(self):
        ...
        params = aimet_torch.defs.SpatialSvdParameters(
            aimet_torch.defs.SpatialSvdParameters.Mode.auto, auto_params, multiplicity=8
        )

        mock_eval = FakeEvaluator(input_shape)
        mock_eval.return_values = [
            0.75,
            0.50,
            0.25,
            0.75,
        ...
        self.assertTrue(
            math.isclose(float(stats.mac_compression_ratio), 0.83, abs_tol=0.01)
        )
    ...
    def test_spatial_svd_compress_auto_multi_input_model(self):
        ...
        self.assertTrue(
            math.isclose(float(stats.mac_compression_ratio), 0.25, abs_tol=0.01)
        )
    ...
    def test_spatial_svd_compress_auto_with_high_multiplicity(self):
        ...
        self.assertEqual((3, 1), compressed_model.layer2[0].conv2[0].kernel_size)
        self.assertTrue(
            math.isclose(float(stats.mac_compression_ratio), 0.87, abs_tol=0.01)
        )
    ...
    def test_weight_svd_compress_manual(self):
        ...
        manual_params = aimet_torch.defs.WeightSvdParameters.ManualModeParams(
            [
                ModuleCompRatioPair(model.layer1[0].conv1, 0.5),
                ModuleCompRatioPair(model.layer2[1].conv2, 0.4),
                ModuleCompRatioPair(model.fc, 0.4),
            ]
        )

        params = aimet_torch.defs.WeightSvdParameters(
            aimet_torch.defs.WeightSvdParameters.Mode.manual, manual_params
        )
    ...
    def test_weight_svd_compress_auto_greedy(self):
        ...
        auto_params = aimet_torch.defs.WeightSvdParameters.AutoModeParams(
            rank_select_scheme=rank_select,
            select_params=greedy_params,
            modules_to_ignore=modules_to_ignore,
        )
        params = aimet_torch.defs.WeightSvdParameters(
            aimet_torch.defs.WeightSvdParameters.Mode.auto, auto_params, multiplicity=8
        )
    ...
    def test_weight_svd_compress_auto_high_multiplicity(self):
        ...
        params = aimet_torch.defs.WeightSvdParameters(
            aimet_torch.defs.WeightSvdParameters.Mode.auto, auto_params, multiplicity=64
        )
    ...
    def test_spatial_svd_with_fine_tuning(self):
        ...
        params = aimet_torch.defs.SpatialSvdParameters(
            aimet_torch.defs.SpatialSvdParameters.Mode.auto, auto_params, multiplicity=1
        )
        results = ModelCompressor.compress_model(
            model,
        ...
        print(stats)
        self.assertTrue(
            math.isclose(float(stats.mac_compression_ratio), 0.7, abs_tol=0.1)
        )
...
class ChannelPruningAcceptanceTests(unittest.TestCase):
    ...
    def test_channel_pruning_manual(self):
        ...
        manual_params = ChannelPruningParameters.ManualModeParams(
            [
                ModuleCompRatioPair(model.layer1[0].conv2, 0.3),
    ...
    @pytest.mark.cuda
    def test_channel_pruning_compress_auto_resnet_custom_downsample_ops_not_allowed(
        self,
    ):
        torch.cuda.empty_cache()
        torch.manual_seed(1)
...
class FakeEvaluator:
    def __init__(self, input_shape):
        ...
        self.rand_inputs = aimet_torch.utils.create_rand_tensors_given_shapes(
            input_shape, torch.device("cpu")
        )
    ...
    def train_model(self, model, layer, train_flag=True):
        ...
        if train_flag:
            mnist_torch_model.train(
                model, epochs=1, use_cuda=True, batch_size=50, batch_callback=None
            )
