...
from aimet_onnx.amp.utils import (
    find_layer_database_for_mac_calculation,
    create_mac_dict,
    find_bit_ops_reduction,
    calculate_running_bit_ops,
    find_param_name_to_parent_name_dict,
    get_quantizer_to_op_type_dict,
)
...
class TestAMPUtils:
    def test_find_layer_database_and_mac_cost(self):
        ...
        for node_name in mac_dict:
            cost = (
                layer_db[node_name].output_shape[2]
                * layer_db[node_name].output_shape[3]
                * layer_db[node_name].weight_shape[0]
                * layer_db[node_name].weight_shape[1]
            )
            if len(layer_db[node_name].weight_shape) == 4:
                cost *= (
                    layer_db[node_name].weight_shape[2]
                    * layer_db[node_name].weight_shape[3]
                )
    ...
    def test_calculate_running_bit_ops(self):
        ...
        quantizer_group = QuantizerGroup(
            parameter_quantizers=("36",), activation_quantizers=("input",)
        )
        ...
        running_bit_ops = calculate_running_bit_ops(
            mac_dict,
            quantizer_group,
            param_op_name_dict,
            op_bitwidth_dict,
        ...
        assert (
            running_bit_ops
            == starting_bit_ops
            + mac_dict["Conv_0"] * new_bw[0][0] * new_bw[1][0]
            - mac_dict["Conv_0"] * max_bw[0][0] * max_bw[1][0]
        )

        # 2) only weight
        quantizer_group = QuantizerGroup(
            parameter_quantizers=("fc.weight",), activation_quantizers=()
        )
        ...
        assert (
            running_bit_ops
            == starting_bit_ops
            + mac_dict["Gemm_14"] * max_bw[0][0] * new_bw[1][0]
    ...
    def test_find_bit_ops_reduction(self):
        ...
        bit_ops_reduction = find_bit_ops_reduction(
            quantizer_group,
            mac_dict,
            param_op_name_dict,
            ((16, None), (16, None)),
    ...
    def test_get_quantizer_to_op_type_dict(self):
        ...
        quantizer_to_op_type = {
            ...
            "onnx::Conv_45": ["Conv"],
            "onnx::Conv_46": ["Conv"],
            "/conv1/Conv_output_0": ["Conv"],
            "/relu1/Relu_output_0": ["Relu"],
            "/maxpool/MaxPool_output_0": ["MaxPool"],
            "onnx::Conv_48": ["Conv"],
            "onnx::Conv_49": ["Conv"],
            "/conv2/Conv_output_0": ["Conv"],
            "/relu2/Relu_output_0": ["Relu"],
            "conv3.weight": ["Conv"],
            "/conv3/Conv_output_0": ["Conv"],
            "conv4.weight": ["Conv"],
            "conv4.bias": ["Conv"],
            "/conv4/Conv_output_0": ["Conv"],
            "/ada/Pad_output_0": ["Pad"],
            "/ada/AveragePool_output_0": ["AveragePool"],
            "/Add_output_0": ["Add"],
            "/relu3/Relu_output_0": ["Relu"],
            "/avgpool/Pad_output_0": ["Pad"],
            "/avgpool/AveragePool_output_0": ["AveragePool"],
            "fc.weight": ["Gemm"],
            "fc.bias": ["Gemm"],
            "output": ["Gemm"],
        }
