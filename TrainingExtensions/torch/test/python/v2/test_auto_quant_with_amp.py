...
from aimet_torch.v2.auto_quant import AutoQuantWithAutoMixedPrecision as AutoQuant
...
class Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer(
            "applied_bn_folding", torch.tensor(False, dtype=torch.bool), persistent=True
        )
        self.register_buffer(
            "applied_cle", torch.tensor(False, dtype=torch.bool), persistent=True
        )
        self.register_buffer(
            "applied_adaround", torch.tensor(False, dtype=torch.bool), persistent=True
        )
        self.register_buffer(
            "applied_amp", torch.tensor(False, dtype=torch.bool), persistent=True
        )
...
_SUCCESS = {"data-visited": "true", "data-stage-result": "success"}
_DISCARDED = {"data-visited": "true", "data-stage-result": "discarded"}
_ERROR_IGNORED = {"data-visited": "true", "data-stage-result": "error-ignored"}
_ERROR_FAILED = {"data-visited": "true", "data-stage-result": "error-failed"}
...
def assert_applied_techniques(
    ...
    assert output_model.applied_adaround == (
        adaround_acc >= max(bn_folded_acc, cle_acc)
    )
    ...
    assert output_model.applied_amp == (
        amp_final_acc >= max(bn_folded_acc, cle_acc, adaround_acc)
    )
...
def patch_ptq_techniques(
    bn_folded_acc, cle_acc, adaround_acc, amp_final_acc, fp32_acc=None, w32_acc=None
):
    if fp32_acc is None:
        fp32_acc = FP32_ACC
    ...
    def mock_eval_callback(model, _):
        if not isinstance(model._conv_0, BaseQuantizationMixin):
            ...
            return fp32_acc
        if model._conv_0.param_quantizers["weight"] is None:
    ...
    with (
        ...
        patch(
            "aimet_torch._base.auto_quant.fold_all_batch_norms", side_effect=bn_folding
        ) as mock_bn_folding,
        patch(
...
class TestAutoQuant:
    @pytest.mark.parametrize(
        "cle_acc, adaround_acc, amp_final_acc", itertools.permutations([0.5, 0.6, 0.7])
    )
    @pytest.mark.parametrize("allowed_accuracy_drop", [0.05, 0.15])
    ...
    @pytest.mark.cuda
    def test_auto_quant_gpu_with_amp(
        self, gpu_model, dummy_input, unlabeled_data_loader
    ):
        bn_folded_acc, cle_acc, adaround_acc, amp_final_acc = 0.4, 0.5, 0.6, 0.7
    ...
    def test_auto_quant_w32_early_exit(
        ...
        with create_tmp_directory() as results_dir:
            with patch_ptq_techniques(
                ...
                output_model, acc, encoding_path, pareto_curve = auto_quant.optimize(
                    allowed_accuracy_drop
                )
    ...
    def test_auto_quant_with_amp_early_exit(
        ...
        with patch_ptq_techniques(
            ...
            def _eval_callback(model, _):
                bw = get_bitwidth(model)
                if bw == _W8A8:
                    return eval_callback(model, _) * 0.996  # Discount eval score
                if bw == _W8A16:
                    return eval_callback(model, _) * 0.998  # Discount eval score
                if bw == _FP16:
                    return eval_callback(model, _) * 0.999  # Discount eval score
                return eval_callback(model, _)

            mocks.eval_callback = _eval_callback

            auto_quant = AutoQuant(
                cpu_model, dummy_input, unlabeled_data_loader, mocks.eval_callback
            )
            ...
            with create_tmp_directory() as results_dir:
                ...
                for enc in (
                    encodings["param_encodings"] + encodings["activation_encodings"]
                ):
                    assert enc["bw"] == 16
                    assert enc["dtype"] == "FLOAT"

    def test_auto_quant_invalid_input(
        self, cpu_model, dummy_input, unlabeled_data_loader
    ):
        auto_quant = AutoQuant(
            cpu_model,
    ...
    def test_auto_quant_fallback(
        ...
        allowed_accuracy_drop = 0.0
        bn_folded_acc, cle_acc, adaround_acc, amp_acc = 0.4, 0.5, 0.6, 0.7
        with patch_ptq_techniques(
            ...
            with create_tmp_directory() as results_dir:
                ...
                with patch(
                    "aimet_torch._base.auto_quant.prepare_model", side_effect=error_fn
                ):
                ...
                with patch(
                    "aimet_torch._base.auto_quant.fold_all_batch_norms",
                ...
                with patch(
                    "aimet_torch._base.auto_quant.equalize_model", side_effect=error_fn
                ):
                ...
                with (
                    ...
                    patch(
                        "aimet_torch._base.auto_quant.equalize_model",
                        side_effect=error_fn,
                    ),
                    patch(
                        "aimet_torch.auto_quant.Adaround._apply_adaround",
    ...
    @pytest.mark.skip(
        reason="GraphModule's custom pickle serialization mechanism doesn't work"
    ...
    def test_auto_quant_caching(
        ...
        with patch_ptq_techniques(
            ...
            with create_tmp_directory() as results_dir:
                ...
                cache_files = [
                    os.path.join(
                        results_dir, ".auto_quant_cache", cache_id, f"{key}.pkl"
                    )
                    for key in (
                        "batchnorm_folding",
                        "cle",
    ...
    def test_auto_quant_adaround_amp_stitching_logic(
        ...
        def run_auto_quant_optimize(param_bw, output_bw, amp_candidates):
            ...
            with patch_ptq_techniques(
                ...
                def _eval_callback(model, _):
                    bw = get_bitwidth(model)
                    if bw == _W4A8:
                        return eval_callback(model, _) * 0.997  # Discount eval score
                    if bw == _W8A8:
                        return eval_callback(model, _) * 0.998  # Discount eval score
                    if bw == _W8A16:
                        return eval_callback(model, _) * 0.999  # Discount eval score
        ...
        """
        mocks = run_auto_quant_optimize(
            param_bw=4, output_bw=8, amp_candidates=[_W4A8, _W8A8, _W8A16, _FP16]
        )
        ...
        """
        mocks = run_auto_quant_optimize(
            param_bw=8, output_bw=8, amp_candidates=[_W8A8, _W8A16, _FP16]
        )
        ...
        """
        mocks = run_auto_quant_optimize(
            param_bw=4, output_bw=8, amp_candidates=[_W4A8, _W8A8, _W8A16]
        )
        ...
        """
        mocks = run_auto_quant_optimize(
            param_bw=4, output_bw=8, amp_candidates=[_W4A8, _W8A8]
        )
        ...
        """
        mocks = run_auto_quant_optimize(
            param_bw=4, output_bw=8, amp_candidates=[_W4A8, _FP16]
        )
        ...
        """
        mocks = run_auto_quant_optimize(param_bw=4, output_bw=8, amp_candidates=[])
...
def get_bitwidth(model: torch.nn.Module):
    ...
    if param_quantizer is None:
        ...
        param_dtype = get_quantizer_quantization_data_type(param_quantizer)

    output_quantizer = model._relu.output_quantizers[0]
    if output_quantizer is None:
        output_bw = 32
        output_dtype = QuantizationDataType.int
    else:
        output_bw = output_quantizer.bitwidth
        output_dtype = get_quantizer_quantization_data_type(output_quantizer)

    return ((output_bw, output_dtype), (param_bw, param_dtype))


def get_quantizer_quantization_data_type(quantizer: QuantizerBase):
    if issubclass(quantizer.__class__, AffineQuantizerBase):
        return QuantizationDataType.int
    elif issubclass(quantizer.__class__, FloatQuantizeDequantize):
        return QuantizationDataType.float
    raise RuntimeError(
        f"Cannot check quantization data type for quantizer class: {quantizer.__class__}"
    )
