...
class TestTrainingExtensionBnFoldToScale:
    ...
    def test_fold_resnet18(self, seed, config):
        ...
        assert (
            not layer2_0.bn1.param_quantizers
            or not layer2_0.bn1.param_quantizers["weight"]
        )
    ...
    def test_fold_bn_after_conv_no_bias(self):
        ...
        assert (
            not model.bn1.param_quantizers or not model.bn1.param_quantizers["weight"]
        )
    ...
    def test_fold_bn_after_linear_layer_no_bias(self):
        ...
        assert (
            ...
            and fc_output_encoding.max == bn_output_quantizer.max
        )
    ...
    def test_fold_bn_after_linear_layer_with_bias(self):
        ...
        fc_output_encoding = model.fc1.output_quantizers[0]
        assert (
            fc_output_encoding.max == bn_output_quantizer.max
            and fc_output_encoding.min == bn_output_quantizer.min
        )
    ...
    def test_fold_auto_mode_with_bn_after_Conv1d_layer(self):
        ...
        assert (
            conv_output_encoding.max == bn_output_quantizer.max
            and conv_output_encoding.min == bn_output_quantizer.min
        )
        conv_output_encoding = model.conv1d.output_quantizers[0]
    ...
    def test_fold_manual_with_bn_after_Conv1d_layer_no_bias(self):
        ...
        assert torch.allclose(
            baseline_output, output_after_fold, atol=conv_output_scale.item()
        )  # Allow 1-tick difference
