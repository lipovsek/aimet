...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda: custom.FloorDivide(), lambda: (arange(0, 6, 0.5), tensor(2))),
        ...
        (lambda: custom.Maximum(), lambda: (randn(100), randn(100))),
        (lambda: custom.Max(), lambda: randn(100)),
        (lambda: custom.AMax(), lambda: randn(100)),
        (lambda: custom.Minimum(), lambda: (randn(100), randn(100))),
        (lambda: custom.Min(), lambda: randn(100)),
        (lambda: custom.AMin(), lambda: randn(100)),
        ...
        (
            lambda: custom.MaskedFill(),
            lambda: (randn(10, 10), randint(0, 1, (10, 10)).bool(), tensor(0.5)),
        ),
        (lambda: custom.Mean(), lambda: randn(10, 10)),
        (lambda: custom.Sum(), lambda: randn(10, 10)),
        (lambda: custom.Prod(), lambda: randint(1, 10, (10,)) / 5),
        (lambda: custom.Log(), lambda: randint(1, 1000, (10, 10))),
        (lambda: custom.Abs(), lambda: randn(100)),
        (lambda: custom.Neg(), lambda: randn(100)),
        (lambda: custom.Argmin(), lambda: randn(100)),
        (lambda: custom.Argmax(), lambda: randn(100)),
        (lambda: custom.ElementwiseCeil(), lambda: randn(100)),
        (lambda: custom.ElementwiseFloor(), lambda: randn(100)),
        (lambda: custom.Asin(), lambda: arange(-1, 1, 0.01)),
        (lambda: custom.Atan(), lambda: randn(100)),
        (lambda: custom.Round(), lambda: randn(100)),
        (
            lambda: custom.Gather(),
            lambda: (randn(10, 10), tensor(1), randint(0, 9, (4, 4))),
        ),
        (
            lambda: custom.LogicalOr(),
            lambda: (
        ...
        (
            lambda: custom.LogicalNot(),
            lambda: torch.randint(0, 2, (10,), dtype=torch.float32),
        ),
        (lambda: custom.Split(), lambda: (randn(10, 10), tensor(2))),
        (
            lambda: custom.Permute(),
            lambda: (randn(10, 10, 1), [tensor(1), tensor(0), tensor(2)]),
        ),
        (lambda: custom.Remainder(), lambda: (randn(100), tensor(1.0))),
        (
            lambda: custom.IndexSelect(),
            lambda: (randn(10, 10), tensor(1), randint(0, 9, (4,))),
        ),
        (lambda: custom.Fmod(), lambda: (randn(10), tensor(1.0))),
        # (lambda: custom.NonZero(),                      lambda: ...),
        # (lambda: custom.TopK(),                         lambda: ...),
        (lambda: custom.Tile(), lambda: (randn(10), [tensor(3)])),
        (
        ...
        (lambda: custom.Square(), lambda: randn(10)),
        (lambda: custom.Select(), lambda: (randn(10, 10), tensor(0), tensor(5))),
        (lambda: custom.Interpolate(), lambda: (randn(3, 10, 10), torch.tensor(20))),
        (
            lambda: custom.MaxPool2d(),
            lambda: (randn(1, 10, 10, 10), [tensor(3), tensor(3)]),
        ),
        (
            lambda: custom.AdaptiveAvgPool2d(),
            lambda: (randn(1, 10, 10), tensor([3, 3])),
        ),
        (
        ...
        (lambda: custom.Pad(), lambda: (randn(10, 10), [tensor(1), tensor(1)])),
        (
        ...
        (lambda: custom.Pow(), lambda: (arange(10.0), tensor(2.0))),
        (lambda: custom.CustomSiLU(), lambda: randn(100)),
        (
            lambda: custom.StridedSlice(),
            lambda: (randn(10), [[tensor(0), tensor(6), tensor(2)]]),
        ),
        (lambda: custom.ChannelShuffle(2), lambda: randn(1, 8, 4, 4)),
        (lambda: custom.CustomGather(), lambda: (randn(10, 10), randint(0, 9, (4, 4)))),
        (lambda: custom.DepthToSpaceCRDMode([2, 2]), lambda: randn(1, 8, 10, 10)),
        (lambda: custom.DepthToSpaceDCRMode(2), lambda: randn(1, 8, 10, 10)),
        # (lambda custom.CustomSparseConv3DLayer(),       lambda: ...),
        # (lambda custom.SparseTensorWrapper(),           lambda: ...),
        # (lambda custom.ScatterDense(),                  lambda: ...),
        (
            lambda: custom.ScatterND(),
            lambda: (randn(10), randint(0, 9, (4, 1)), randn(4)),
        ),
        (
            lambda: custom.RoiAlign(8, 1.0, -1),
            lambda: (
                torch.randn(1, 3, 10, 10),
        ...
        (lambda: custom.GatherNd(0), lambda: (randn(5, 5), randint(0, 5, (2,)))),
        (
            lambda: custom.ScatterElements(dim=0),
            lambda: (randn(3, 4, 4), randint(0, 3, (3, 4, 4)), randn(3, 4, 4)),
        ),
        (lambda: custom.OneHot(20, 0.0, 0.9), lambda: randint(0, 20, (10,))),
        (lambda: custom.Expand(), lambda: (randn(10), [tensor(5), tensor(10)])),
        (
            lambda: custom.DynamicLinear(),
            lambda: (torch.randn(10, 10), torch.randn(10, 10), torch.randn(10)),
        ),
    ],
)
