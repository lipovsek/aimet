...
class TestQuantAnalyzer:
    def test_check_model_sensitivity_to_quantization(self):
        ...
        fp32_acc, weight_quantized_acc, act_quantized_acc = (
            quant_analyzer.check_model_sensitivity_to_quantization(sim)
        )
    ...
    def test_sort_quant_wrappers_based_on_occurrence(self):
        ...
        sim.compute_encodings(evaluate, dummy_input)
        quant_analyzer = QuantAnalyzer(
            model, dummy_input, CallbackFunc(None), CallbackFunc(None)
        )
        sorted_quant_wrappers_dict = (
            quant_analyzer._sort_quant_wrappers_based_on_occurrence(sim)
        )
    ...
    def test_get_enabled_quantizers(self):
        ...
        enabled_quant_wrappers = quant_analyzer._get_enabled_quantizers(
            sorted_quant_wrappers_dict
        )
        ...
        for quant_wrapper, enabled_quantizers in enabled_quant_wrappers.items():
            assert isinstance(quant_wrapper, BaseQuantizationMixin)
            assert all(
                isinstance(quantizer, QuantizerBase) for quantizer in enabled_quantizers
            )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for quant_wrapper_name in layer_wise_eval_score_dict.keys():
                ...
                assert os.path.isfile(
                    os.path.join(tmp_dir, "per_layer_quant_enabled.html")
                )
    ...
    def test_perform_per_layer_analysis_by_disabling_quant_wrappers(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Check if it is exported to correct html file.
            assert os.path.isfile(
                os.path.join(tmp_dir, "per_layer_quant_disabled.html")
            )
    ...
    def test_export_per_layer_stats_histogram(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert os.path.isfile(
                os.path.join(tmp_dir, "activations_pdf/conv1_input_q0_0.html")
            )
            assert os.path.isfile(
    ...
    def test_export_per_layer_stats_histogram_per_channel(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert os.path.exists(os.path.join(tmp_dir, "weights_pdf"))
            assert os.path.isfile(
                os.path.join(tmp_dir, "activations_pdf/bn1_output_q0_0.html")
            )
            assert os.path.isfile(
                os.path.join(tmp_dir, "weights_pdf/conv1/conv1_weight_0.html")
            )
            assert os.path.isfile(
                os.path.join(tmp_dir, "weights_pdf/conv1/conv1_weight_31.html")
            )
            assert os.path.isfile(
                os.path.join(tmp_dir, "weights_pdf/conv2/conv2_weight_0.html")
            )
            assert os.path.isfile(
                os.path.join(tmp_dir, "weights_pdf/conv2/conv2_weight_15.html")
            )
    ...
    def test_export_per_layer_encoding_min_max_range(self):
        ...
        model = TinyModel().eval()
        sim = QuantizationSimModel(
            model, dummy_input, config_file=get_path_for_per_tensor_config()
        )
        sim.compute_encodings(evaluate, dummy_input)
        forward_pass_callback = CallbackFunc(calibrate, dummy_input)
        eval_callback = CallbackFunc(evaluate, dummy_input)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert os.path.isfile(
                os.path.join(tmp_dir, "min_max_ranges/activations.html")
            )
    ...
    def test_export_per_layer_encoding_min_max_range_per_channel(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert os.path.isfile(
                os.path.join(tmp_dir, "min_max_ranges/conv1_weight.html")
            )
    ...
    def test_exclude_modules_from_quantization(self):
        ...
        QuantAnalyzer._exclude_modules_from_quantization(
            model, sim, modules_to_ignore=[model.conv1, model.fc]
        )
