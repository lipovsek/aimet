...
def patch_ptq_techniques(
    ...
    def mock_eval_callback(model, _):
        if not isinstance(model._conv_0, BaseQuantizationMixin):
            ...
            return fp32_acc
        if (
            not model._conv_0.param_quantizers["weight"]
            or model._conv_0.param_quantizers["weight"].bitwidth == 32
        ):
...
class TestAutoQuant:
    def test_auto_quant_run_inference(
    ...
    @pytest.mark.parametrize(
        "bn_folded_acc, cle_acc, adaround_acc", itertools.permutations([0.5, 0.6, 0.7])
    )
    ...
    def test_consecutive_calls(self, cpu_model, dummy_input, unlabeled_data_loader):
        ...
        with patch_ptq_techniques(bn_folded_acc, cle_acc, adaround_acc) as mocks:
            with patch(
                "aimet_torch._base.auto_quant.prepare_model", side_effect=prepare_model
                ...
                self._do_test_optimize_auto_quant(
                    auto_quant, cpu_model, 0.0, bn_folded_acc, cle_acc, adaround_acc
                )
                assert prepare_model_mock.call_count == 1
                assert mocks.fold_all_batch_norms.call_count == 1
                assert mocks.equalize_model.call_count == 1
    ...
    def test_auto_quant_invalid_input(
        ...
        with pytest.raises(ValueError):
            AutoQuant(
                cpu_model, dummy_input, unlabeled_data_loader, lambda: None, param_bw=1
            )

        with pytest.raises(ValueError):
            AutoQuant(
                cpu_model, dummy_input, unlabeled_data_loader, lambda: None, param_bw=64
            )

        with pytest.raises(ValueError):
            AutoQuant(
                cpu_model, dummy_input, unlabeled_data_loader, lambda: None, output_bw=2
            )
    ...
    def test_auto_quant_inference_fallback(
        ...
        with patch_ptq_techniques(
            ...
            with tempfile.TemporaryDirectory() as results_dir:
                ...
                with patch(
                    "aimet_torch._base.auto_quant.ModelValidator.validate_model",
                ...
                with (
                    patch(
                        "aimet_torch._base.auto_quant.prepare_model",
                        side_effect=error_fn,
                    ),
                    patch(
                        "aimet_torch._base.auto_quant.ModelValidator.validate_model",
    ...
    def test_auto_quant_early_exit(self, cpu_model, dummy_input, unlabeled_data_loader):
        ...
        with tempfile.TemporaryDirectory() as results_dir:
            with patch_ptq_techniques(
                ...
                output_model, acc, encoding_path = auto_quant.optimize(
                    allowed_accuracy_drop
                )
        ...
        with patch_ptq_techniques(bn_folded_acc, cle_acc, adaround_acc) as mocks:
            ...
            def eval_callback(model, _):
                ...
                if isinstance(model._conv_0, BaseQuantizationMixin):
                    if model._conv_0.param_quantizers["weight"] and not isinstance(
                        model._conv_0.param_quantizers["weight"].encoding_analyzer,
                        encoding_analyzer.SqnrEncodingAnalyzer,
                    ):
                        return 0.0
                    if model._relu.output_quantizers[0] and not isinstance(
                        model._relu.output_quantizers[0].encoding_analyzer,
            ...
            def optimize(self, *args, **kwargs):
                ...
                assert (
                    self._quantsim_params["quant_scheme"].param_quant_scheme
                    == QuantScheme.post_training_tf_enhanced
                )
                assert (
                    self._quantsim_params["quant_scheme"].output_quant_scheme
    ...
    def test_set_additional_params(self, cpu_model, dummy_input, unlabeled_data_loader):
        ...
        adaround_acc = 0.3
        with (
            patch_ptq_techniques(bn_folded_acc, cle_acc, adaround_acc) as mocks,
            set_encoding_version("0.6.1"),
        ):
    ...
    def test_auto_quant_nested_input_quantizers(self):
        ...
        auto_quant = AutoQuant(model, dummy_input, data_loader, lambda *args: 1.0)
