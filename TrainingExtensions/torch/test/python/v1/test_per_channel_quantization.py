...
class TestPerChannelQcQuantizeOpStaticGrid:
    ...
    def test_per_channel_asymmetric_qdq(self):
        ...
        expected_out = torch.tensor(
            [
                [-3.0, -3.0, -3.0, 0, 0.098, 2.0],
                [-3.0, -3.0, -3.0, 0, 0.098, 2.0],
                [-3.0, -3.0, -3.0, 0, 0.098, 2.0],
                [-5.9953, -5.0070, -2.9976, 0, 0.09888, 2.4047],
            ],
            dtype=torch.float32,
        )
    ...
    def test_per_channel_symmetric_compute_encodings(self):
        ...
        assert torch.allclose(
            torch.tensor(quantizer.encoding[0].max),
            torch.tensor(7 - quantizer.encoding[0].delta),
        )

        assert quantizer.encoding[3].min == -5
        assert torch.allclose(
            torch.tensor(quantizer.encoding[3].max),
            torch.tensor(5 - quantizer.encoding[3].delta),
        )
    ...
    def test_model_with_two_inputs_per_channel(self):
        ...
        assert isinstance(
            sim.model.conv1_a.param_quantizers["weight"], StaticGridPerChannelQuantizer
        )
        assert isinstance(
            sim.model.conv1_a.param_quantizers["bias"], StaticGridPerChannelQuantizer
        )
        assert isinstance(
            sim.model.conv1_a.output_quantizers[0], StaticGridPerTensorQuantizer
        )

        assert isinstance(
            sim.model.fc2.param_quantizers["weight"], StaticGridPerChannelQuantizer
        )
        assert isinstance(
            sim.model.fc2.param_quantizers["bias"], StaticGridPerChannelQuantizer
        )
        assert isinstance(
            sim.model.fc2.output_quantizers[0], StaticGridPerTensorQuantizer
        )
        ...
        assert (
            sim.model.conv1_a.param_quantizers["weight"].encoding[0]
            != sim.model.conv1_a.param_quantizers["weight"].encoding[1]
        )
        assert (
            sim.model.fc2.param_quantizers["weight"].encoding[0]
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with open(
                os.path.join(tmp_dir, "two_input_model_per_channel.encodings"), "r"
            ) as encodings_file:
    ...
    def test_transposed_conv_layer_per_channel(self):
        ...
        quantization_wrapper = StaticGridQuantWrapper(
            layer,
            weight_bw=8,
            activation_bw=8,
            round_mode="nearest",
        ...
        weight_quantizer.update_encoding_stats(
            quantization_wrapper._module_to_wrap.weight
        )
        # Assign golden vector to encodings
        weight_quantizer.encoding = encodings
        round_mode = libpymo.RoundingMode.ROUND_NEAREST
        # Quantize Dequantize
        output = weight_quantizer.quantize_dequantize(
            quantization_wrapper._module_to_wrap.weight, round_mode
        )
...
class TestPerChannelQcQuantizeOpLearnedGrid:
    ...
    def test_tensor_quantizer(self):
        ...
        for _ in range(3):
            ...
            encoding.bw, encoding.max, encoding.min, encoding.delta, encoding.offset = (
                8,
                3,
                -2,
                1,
    ...
    def test_quantize_dequantize_tensor_quantizer(self):
        ...
        tensor[0, :, :, :] *= 1.5
        tensor[2, :, :, :] *= 4
        quant_dequantized_output = tensor_quantizer.quantize_dequantize(
    ...
    def test_compute_gradients_Parameter_Quantizer(self, dtype):
        ...
        encoding_min = torch.nn.Parameter(
            torch.FloatTensor([0.0, 0.0, 0.0]).to("cuda", dtype), requires_grad=True
        )
        encoding_max = torch.nn.Parameter(
            torch.FloatTensor([1.0, 2.5, 3.5]).to("cuda", dtype), requires_grad=True
        )
        ...
        _, intermediate_result = calculate_forward_pass(
            tensor, param_quantizer, encoding_min, encoding_max
        )
        enc_min_grad, enc_max_grad = ParameterQuantizer.compute_gradients(
            tensor, grad, intermediate_result, param_quantizer.channel_axis
        )
    ...
    def test_export_model_with_two_inputs(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(
                os.path.join(temp_dir, "two_input_model_per_channel.encodings"), "r"
            ) as encodings_file:
                encodings = json.load(encodings_file)
            param_encodings = {
                encoding["name"]: encoding for encoding in encodings["param_encodings"]
    ...
    def test_export_model_with_two_inputs_fp16(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(
                os.path.join(temp_dir, "results.encodings"), "r"
            ) as encodings_file:
                encodings = json.load(encodings_file)
    ...
    def test_model_with_two_inputs_in_manual_mixed_precision_mode(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config_file = save_config_file_for_per_channel_quantization(Path(temp_dir))
            sim = QuantizationSimModel(
                model, dummy_input=dummy_input, config_file=config_file
            )

            sim.model.conv1_a.param_quantizers["weight"].bitwidth = 16
            sim.model.conv1_a.param_quantizers[
                "weight"
...
def create_learned_grid_wrapper():
    ...
    wrapper = LearnedGridQuantWrapper(
        conv1,
        round_mode="nearest",
        quant_scheme=QuantScheme.training_range_learning_with_tf_init,
        is_symmetric=False,
