...
class TestTrainingExtensionsGraphMeta(unittest.TestCase):
    def test_duplicate_node_name(self):
        ...
        full_name, namespc, name, typ, parms = self._split_test_node_attrs(
            INPUT_NODES[-1]
        )
    ...
    def test_valid_meta(self):
        ...
        assert sum(stats["nodes"] for stats in stats_by_type.values()) == len(
            INPUT_NODES
        )
        assert (
            sum(stats["parms"] for stats in stats_by_type.values())
            == meta.num_parameters
        )
        ...
        assert sum(stats["uses"] for stats in stats_by_type.values()) == len(
            INVOCATIONS
        )
        assert sum(stats["macs"] for stats in stats_by_type.values()) == sum(
            entry[1] for entry in INVOCATIONS
        )
    ...
    def create_test_meta(cls, model_name="TestModel", data=None):
        ...
        if not data:
            data = INPUT_NODES
        for node in data:
            full_name, namespc, name, typ, parms_static = cls._split_test_node_attrs(
                node, prefix=model_name
            )
