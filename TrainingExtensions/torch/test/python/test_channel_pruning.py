...
class TestTrainingExtensionsChannelPruning(unittest.TestCase):
    def test_find_input_data_pixel_indices(self):
        ...
        for kernel_size, padding, stride in itertools.product(*all_options):
            ...
            size_options = [
                [x, y] for x in range(out_height_max) for y in range(out_width_max)
            ]
            ...
            for size_opt in size_options:
                ...
                in_data_height_range, in_data_width_range = (
                    InputMatchSearch._find_pixel_range_for_input_data(
                ...
                self.assertEqual(
                    in_data_width_range[1],
                    (width * layer.stride[1]) - layer.padding[1] + layer.kernel_size[1],
                )
    ...
    def test_find_input_match(self):
        ...
        self.assertEqual(
            np.sum(input_match), 27 + 28 + 29 + 35 + 36 + 37 + 43 + 44 + 45
        )
    ...
    def test__determine_output_pixel_height_width_range_for_random_selection(self):
        ...
        for kernel_size, stride, padding in itertools.product(*all_options):
            ...
            if kernel_size[0] >= padding[0]:
                ...
                assert start == padding[0] and end == (
                    output_data_shape[2] - padding[0]
                )
            ...
            if kernel_size[1] >= padding[1]:
                ...
                assert start == padding[1] and end == (
                    output_data_shape[3] - padding[1]
                )
    ...
    def test_find_input_match_for_pixel_from_output_data_baseline(self):
        ...
        heights = np.random.choice(
            range(2, (max_height - 2)), size=[num_output_pixels], replace=True
        )
        widths = np.random.choice(
            range(2, (max_width - 2)), size=[num_output_pixels], replace=True
        )
        ...
        for kernel_size, padding, size_opt, stride in itertools.product(*all_options):
            ...
            layer_attributes = (
                conv_filter.kernel_size,
                conv_filter.stride,
                conv_filter.padding,
            )

            conv_filter.weight.data = torch.FloatTensor(
                np.ones([1, 1, kernel_size[0], kernel_size[1]], dtype=np.float32)
            )

            input_match = InputMatchSearch._find_input_match_for_output_pixel(
                input_frame[batch_num], layer_attributes, output_data_pixel
            )
            conv2d_out = functional.conv2d(
                torch.FloatTensor(input_frame),
    ...
    def test_get_activation_data(self):
        ...
        for batch in range(number_of_batches):
            ...
            self.assertTrue(
                np.array_equal(
                    conv2_input_copy.cpu().detach().numpy(),
                    conv2_input_data[
                        batch * batch_size : (batch + 1) * batch_size, :, :, :
                    ],
                )
    ...
    def test_subsample_data(self, np_choice_function):
        ...
        self.assertTrue(
            np.array_equal(sub_sample_input[0, :, :, :], input_data[0, :, 1:6, 1:6])
        )
        self.assertTrue(
            np.array_equal(sub_sample_input[1, :, :, :], input_data[1, :, 1:6, 1:6])
        )
        ...
        self.assertTrue(
            np.array_equal(
                sub_sample_output, output_data[:, :, output_pixel[0], output_pixel[1]]
            )
        )
    ...
    def test_linear_regression(self):
        ...
        reshaped_input_data = input_data.reshape(
            number_of_images, in_channels * k_h * k_w
        )

        # calculate y = x * w
        output_data = np.matmul(reshaped_input_data, weight)

        new_w, _ = WeightReconstructor._linear_regression(
            input_data=reshaped_input_data, output_data=output_data, bias=False
        )
        ...
        # fit_intercept = True
        new_w, new_b = WeightReconstructor._linear_regression(
            input_data=reshaped_input_data, output_data=output_data, bias=True
        )
        new_w = new_w.reshape(new_w.shape[0], np.prod(new_w.shape[1:4])).transpose()
    ...
    def test_data_sub_sampling_and_reconstruction(self):
        ...
        cp._data_subsample_and_reconstruction(
            orig_layer=orig_model.conv2,
            pruned_layer=comp_model.conv2,
            orig_model=orig_model,
            comp_model=comp_model,
        ...
        self.assertEqual(
            comp_model.conv2.weight.data.shape, orig_model.conv2.weight.data.shape
        )
        self.assertEqual(
            comp_model.conv2.bias.data.shape, orig_model.conv2.bias.data.shape
        )
        ...
        self.assertTrue(
            np.allclose(
                comp_model.conv2.weight.data.cpu().detach().numpy(),
                orig_model.conv2.weight.data.cpu().detach().numpy(),
                atol=1e-0,
            )
        )
        ...
        self.assertTrue(
            np.allclose(
                comp_model.conv2.bias.data.cpu().detach().numpy(),
                orig_model.conv2.bias.data.cpu().detach().numpy(),
                atol=1e-0,
            )
        )
    ...
    def test_data_sub_sample_and_reconstruction_with_zero_channels(self):
        ...
        self.assertFalse(
            np.allclose(
                before_reconstruction.cpu().detach().numpy(),
                after_reconstruction.cpu().detach().numpy(),
            )
        )
    ...
    def test_data_sub_sampling_and_reconstruction_without_bias(self):
        ...
        WeightReconstructor.reconstruct_params_for_conv2d(
            layer=conv_layer, input_data=sampled_inp_data, output_data=sampled_out_data
        )
        ...
        self.assertTrue(
            np.allclose(
                new_weight.cpu().detach().numpy(),
                orig_weight.cpu().detach().numpy(),
                atol=1e-0,
            )
        )
    ...
    def test_select_inp_channels(self):
        ...
        orig_model.conv2.weight.data = torch.FloatTensor(
            np.array(range(32 * 64 * 5 * 5)).reshape(64, 32, 5, 5)
        )
        ...
        for comp_ratio, remaining_channels in comp_ratio_prune_inp_channels_list:
            prune_indices = cp._select_inp_channels(
                orig_model.conv2, comp_ratio=comp_ratio
            )
    ...
    def test_prune_layer(self):
        ...
        dummy_input = create_rand_tensors_given_shapes(
            input_shape, get_device(orig_model)
        )
        ...
        input_channel_pruner._prune_layer(
            orig_layer_db, comp_layer_db, conv2, 0.5, CostMetric.mac
        )
    ...
    def test_prune_layer_with_seq(self):
        ...
        data_loader = create_fake_data_loader(
            dataset_size=dataset_size, batch_size=batch_size, image_size=(3, 224, 224)
        )
        ...
        input_channel_pruner._prune_layer(
            orig_layer_db, comp_layer_db, conv_below_split, 0.25, CostMetric.mac
        )
        ...
        self.assertEqual(
            list(comp_layer_db.model.layer1[1].conv1[1].weight.shape), [64, 16, 3, 3]
        )
    ...
    def test_prune_model(self):
        ...
        comp_layer_db = input_channel_pruner.prune_model(
            orig_layer_db, layer_comp_ratio_list, CostMetric.mac, trainer=None
        )
    ...
    def test_prune_model_with_seq(self):
        ...
        layer_comp_ratio_list = [
            LayerCompRatioPair(
                Layer(model.layer4[1].conv1, "layer4.1.conv1", None), 0.5
            ),
            LayerCompRatioPair(
                Layer(model.layer3[1].conv1, "layer3.1.conv1", None), 0.5
            ),
            LayerCompRatioPair(
                Layer(model.layer2[1].conv1, "layer2.1.conv1", None), 0.5
            ),
            LayerCompRatioPair(
                Layer(model.layer1[1].conv1, "layer1.1.conv1", None), 0.5
            ),
            LayerCompRatioPair(
                Layer(model.layer1[0].conv2, "layer1.0.conv2", None), 0.5
            ),
        ]
        ...
        self.assertEqual(
            list(comp_layer_db.model.layer1[0].conv2.weight.shape), [64, 32, 3, 3]
        )
        # impacted
        ...
        self.assertEqual(
            list(comp_layer_db.model.layer1[0].conv1.weight.shape), [32, 64, 3, 3]
        )
        ...
        self.assertEqual(comp_layer_db.model.layer1[1].conv1[1].out_channels, 64)
        self.assertEqual(
            list(comp_layer_db.model.layer1[1].conv1[1].weight.shape), [64, 32, 3, 3]
        )
        ...
        self.assertEqual(
            list(comp_layer_db.model.layer2[1].conv1[1].weight.shape), [128, 64, 3, 3]
        )
        ...
        self.assertEqual(
            list(comp_layer_db.model.layer3[1].conv1[1].weight.shape), [256, 128, 3, 3]
        )
