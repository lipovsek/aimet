...
TensorQuantizersTupleType = Tuple[
    List[LazyQuantizer], List[LazyQuantizer], List[LazyQuantizer], List[LazyQuantizer]
]
...
class SupergroupConfigCallback(AimetCommonSupergroupConfigCallback):
    ...
    def __init__(
        self,
        module_to_quantsim_wrapper_dict: Dict[torch.nn.Module, LazyQuantizeWrapper],
    ):
    ...
    def __call__(self, _, op_list: List[Op]):
        ...
        for index, op in enumerate(op_list):
            ...
            if index == 0:
                ...
                first_quantsim_wrapper = self._module_to_quantsim_wrapper_dict[
                    op_list[0].get_module()
                ]
                ...
                last_quantsim_wrapper = self._module_to_quantsim_wrapper_dict[
                    op_list[-1].get_module()
                ]

                for quantizer in last_quantsim_wrapper.input_quantizers:
                    quantizer.enabled = False
            else:
                quantsim_wrapper = self._module_to_quantsim_wrapper_dict[
                    op.get_module()
                ]

                for quantizer in (
                    quantsim_wrapper.input_quantizers
                    + quantsim_wrapper.output_quantizers
                ):
                    quantizer.enabled = False
...
class QuantSimConfigurator(AimetCommonQuantSimConfigurator):
    ...
    def __init__(
        ...
        self._module_to_quantsim_wrapper_dict = _create_module_to_quantsim_wrapper_dict(
            model
        )
        self._named_modules_to_tensor_quantizers_dict = (
            self._create_named_modules_to_tensor_quantizers_dict()
        )
        self._elementwise_op_to_tensor_quantizers_dict = (
            self._create_elementwise_op_to_tensor_quantizers_dict()
        )
        ...
        if ENFORCE_TARGET_DTYPE_BITWIDTH_CONFIG:
            if self.check_correctness_of_dtype_bw_rules(
                ...
                logger.info(
                    "Supported Kernel check for valid dtype and bitwidth overrides completed"
                )
    ...
    def _create_named_modules_to_tensor_quantizers_dict(
        self,
    ) -> Dict[torch.nn.Module, TensorQuantizersTupleType]:
        """
        ...
        for module, quantsim_wrapper in self._module_to_quantsim_wrapper_dict.items():
            ...
            if op:
                ...
                output_true_list = self._get_tensor_quantizers_for_output_true_setting(
                    op
                )
                input_false_list = self._get_tensor_quantizers_for_input_false_setting(
                    op
                )
                output_false_list = (
                    self._get_tensor_quantizers_for_output_false_setting(op)
                )
            else:
            ...
            module_to_tensor_quantizers_dict[module] = (
                input_true_list,
                output_true_list,
                input_false_list,
                output_false_list,
    ...
    def _create_elementwise_op_to_tensor_quantizers_dict(
        self,
    ) -> Dict[Op, TensorQuantizersTupleType]:
        """
        ...
        for op in elementwise_ops:
            ...
            module_to_tensor_quantizers_dict[op] = (
                input_true_list,
                output_true_list,
                input_false_list,
                output_false_list,
    ...
    def _get_tensor_quantizers_for_input_true_setting(
        self, op: Op
    ) -> List[LazyQuantizer]:
        """
            ...
            while queue:
                ...
                if current_op.inputs:
                    input_ops = [
                        ...
                        if not inp.is_model_input and not inp.is_const
                    ]
                    for input_op in input_ops:
                        if (
                            input_op.get_module() is not None
                            and input_op.get_module()
                            in self._module_to_quantsim_wrapper_dict
                        ):
                            qc_quantize_wrapper = self._module_to_quantsim_wrapper_dict[
                                input_op.get_module()
                            ]
                            tensor_quantizers_for_input_true.append(
                                qc_quantize_wrapper.output_quantizers[0]
                            )
                        elif input_op.type == CG_SPLIT:
                            queue.append(input_op)
        return tensor_quantizers_for_input_true

    def _get_tensor_quantizers_for_output_true_setting(
        self, op: Op
    ) -> List[LazyQuantizer]:
        """
            ...
            while queue:
                ...
                if current_op.outputs:
                    ...
                    for output_op in output_ops:
                        if (
                            output_op.get_module() is not None
                            and output_op.get_module()
                            in self._module_to_quantsim_wrapper_dict
                        ):
                            qc_quantize_wrapper = self._module_to_quantsim_wrapper_dict[
                                output_op.get_module()
                            ]
                            tensor_quantizers_for_output_true += (
                                qc_quantize_wrapper.input_quantizers
                            )
                        elif output_op.type == CG_SPLIT:
                            queue.append(output_op)
        return tensor_quantizers_for_output_true

    def _get_tensor_quantizers_for_input_false_setting(
        self, op: Op
    ) -> List[LazyQuantizer]:
        """
        Get a list of tensor quantizers that would be affected if the given op is specified to have input quantization
        ...
        neighboring_input_ops = get_all_ops_in_neighborhood(
            op, "input", split_type=CG_SPLIT
        )
        for input_op in neighboring_input_ops:
            if (
                input_op.type != CG_SPLIT
                and input_op.get_module() is not None
                and input_op.get_module() in self._module_to_quantsim_wrapper_dict
            ):
                qc_quantize_wrapper = self._module_to_quantsim_wrapper_dict[
                    input_op.get_module()
                ]
                if neighboring_input_ops[input_op] == "input":
                    ...
                    tensor_quantizers_for_input_false += (
                        qc_quantize_wrapper.output_quantizers
                    )
        return tensor_quantizers_for_input_false

    def _get_tensor_quantizers_for_output_false_setting(
        self, op: Op
    ) -> List[LazyQuantizer]:
        """
        Get a list of tensor quantizers that would be affected if the given op is specified to have output quantization
        ...
        neighboring_output_ops = get_all_ops_in_neighborhood(
            op, "output", split_type=CG_SPLIT
        )
        for output_op in neighboring_output_ops:
            if (
                output_op.type != CG_SPLIT
                and output_op.get_module() is not None
                and output_op.get_module() in self._module_to_quantsim_wrapper_dict
            ):
                qc_quantize_wrapper = self._module_to_quantsim_wrapper_dict[
                    output_op.get_module()
                ]
    ...
    def _disable_all_quantizers(self):
        ...
        for quantsim_wrapper in self._module_to_quantsim_wrapper_dict.values():
            for quantizer in (
                quantsim_wrapper.input_quantizers + quantsim_wrapper.output_quantizers
            ):
                quantizer.enabled = False
    ...
    def _set_default_configs(self, default_configs: DefaultsType):
        ...
        if (
            ENFORCE_TARGET_DTYPE_BITWIDTH_CONFIG
            and ConfigDictKeys.SUPPORTED_KERNELS in default_configs
        ):
    ...
    def _set_default_configs_for_ops(self, default_op_configs: ConfigType):
        ...
        ) in self._named_modules_to_tensor_quantizers_dict.values():
        ...
        for (
            input_output_tensor_quantizers
        ) in self._elementwise_op_to_tensor_quantizers_dict.values():
            self._set_config_for_module(
                input_output_tensor_quantizers,
                default_op_configs,
                modified_tensor_quantizers,
    ...
    def _set_param_configs(self, param_configs: ParamType):
        ...
        for quantsim_wrapper in self._module_to_quantsim_wrapper_dict.values():
            for param_name in quantsim_wrapper.param_quantizers.keys():
                quantsim_param_name = MAP_PYTORCH_PARAM_NAME_TO_QUANTSIM_NAME.get(
                    param_name, None
                )
                if (
                    quantsim_param_name is not None
                    and quantsim_param_name in param_configs
                ):
                    param_config = param_configs[quantsim_param_name]
                    _set_config_for_param(
                        quantsim_wrapper.param_quantizers[param_name], param_config
                    )
    ...
    def _merge_op_types_info(self, op_configs: OpTypeType):
        ...
        for module, _ in self._named_modules_to_tensor_quantizers_dict.items():
            ...
            if (
                onnx_types
                and backend_type in op_configs
                and backend_type not in merged_backend_types
            ):
                ...
                for onnx_type in onnx_types:
                    if onnx_type in op_configs and backend_type != onnx_type:
                        ...
                        op_configs[backend_type] = merge_op_type_config(
                            backend_type_op_config, onnx_op_config
                        )
    ...
    def _set_config_for_module(
        self,
        input_output_tensor_quantizers: TensorQuantizersTupleType,
        op_config: OpType,
        modified_tensor_quantizers: Dict[LazyQuantizer, Set],
        ...
        if ConfigDictKeys.PARAMS in op_config:
            ...
            for param_name in quantsim_wrapper.param_quantizers.keys():
                ...
                if (
                    quantsim_param_name is not None
                    and quantsim_param_name in op_config[ConfigDictKeys.PARAMS]
                ):
        ...
        if (
            ENFORCE_TARGET_DTYPE_BITWIDTH_CONFIG
            and ConfigDictKeys.SUPPORTED_KERNELS in op_config
        ):
            if module is None:
                logger.error("No module provided to set params for")
                raise AssertionError("No module provided to set params for")
    ...
    def _set_supergroup_configs(self, supergroups_configs: List[SupergroupType]):
        ...
        def find_scale_foldable_bns(cg):
            ...
            for op_type in conv_types + linear_types:
                patterns_with_callbacks.append(
                    PatternType(pattern=[op_type, "BatchNormalization"], action=handler)
                )
        ...
        def fuse_config(conv: Op, bn: Op):
            ...
            for conv_quantizer, bn_quantizer in zip(
    ...
    def _set_model_output_configs(self, model_output_configs: ConfigType):
        ...
        for op in output_ops:
            if op.get_module() in self._named_modules_to_tensor_quantizers_dict:
                modified_tensor_quantizers = {}
                self._set_config_for_module(
                    self._named_modules_to_tensor_quantizers_dict[op.get_module()],
    ...
    def _override_param_bw_dtype(
        self,
        quantize_wrapper: LazyQuantizeWrapper,
        data_type: QuantizationDataType = None,
        bitwidth: int = None,
    ):
        """
    ...
    def _override_act_bw_dtype(
        ...
        if data_type == QuantizationDataType.float and bitwidth == 16:
            # pylint: disable=protected-access
            conn_graph_op = self._conn_graph._module_to_op_dict[
                quantize_wrapper._module_to_wrap
            ]
            ...
            for input_op in input_ops:
                if not self._op_type_default_override_supported_kernel_lookup(
                    input_op.type, bitwidth, data_type
                ):
            ...
            for output_op in output_ops:
                if not self._op_type_default_override_supported_kernel_lookup(
                    output_op.type, bitwidth, data_type
                ):
            ...
            for quantizer in (
                quantize_wrapper.input_quantizers + quantize_wrapper.output_quantizers
            ):
    ...
    def _generate_and_apply_op_instance_specific_config(self):
        ...
        for op in self._conn_graph.ordered_ops:
            if op.get_module() in self._module_to_quantsim_wrapper_dict:
                ...
                wrapper.supported_kernels, per_channel_quantization = (
...
def merge_op_type_config(
    backend_type_op_config: Dict, onnx_type_op_config: Dict
) -> Dict:
    """
    ...
    """
    backend_type_op_config_supported_kernels = backend_type_op_config.get(
    ...
    if backend_type_op_config_supported_kernels:
        backend_type_op_config[ConfigDictKeys.SUPPORTED_KERNELS] = (
            backend_type_op_config_supported_kernels
        )
...
def _create_module_to_quantsim_wrapper_dict(
    model: torch.nn.Module,
) -> Dict[torch.nn.Module, LazyQuantizeWrapper]:
    """
...
def _set_config_for_param(param_quantizer: LazyQuantizer, param_config: ConfigType):
    """
...
def _modify_tensor_quantizers(
    ...
    tensor_quantizers_to_modify = _get_tensor_quantizers_to_modify(
        input_output_tensor_quantizers, setting_name, quantizer_setting
    )
    for tensor_quantizer in tensor_quantizers_to_modify:
        if (
            tensor_quantizer in modified_tensor_quantizers
            and setting_type in modified_tensor_quantizers[tensor_quantizer]
        ):
            ...
            if setting_name in [
                ...
                current_setting = {
                    ConfigDictKeys.MIN: tensor_quantizer.encoding_min_max_fixed_vals[0],
...
def _get_tensor_quantizers_to_modify(
    input_output_tensor_quantizers: TensorQuantizersTupleType,
    setting_name: str,
    quantizer_setting: bool,
) -> List[LazyQuantizer]:
    """
