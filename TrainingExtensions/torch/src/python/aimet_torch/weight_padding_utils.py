...
def weight_pad(
    sim: _QuantizationSimModelInterface, layer_bw_dict: Dict[str, WeightPaddingParams]
):
    """
    ...
    for layer_name, layer in sim.named_qmodules():
        ...
        if (
            "weight" in param_quant_dict
            and bw_values.target_kernel_bw > bw_values.simulated_bw
        ):
            ...
            quant_dequant_weight = param_weight_quant.quantize_dequantize(
                layer_weight, MAP_ROUND_MODE_TO_PYMO["nearest"]
            )
            # update weights
            layer._module_to_wrap.weight.data = quant_dequant_weight

            # recompute encodings to adjust for quantization scale
            param_weight_quant.encoding = recompute_encodings(
                param_weight_quant, bw_values
            )
...
def recompute_encodings(quantizer: _QuantizerProtocol, bw_params: WeightPaddingParams):
    ...
    for index, encoding in enumerate(encoding_list):
        ...
        adjusted_quantizer = libpymo.TensorQuantizer(
            MAP_QUANT_SCHEME_TO_PYMO[quantizer.quant_scheme],
            MAP_ROUND_MODE_TO_PYMO["nearest"],
        )
        adjusted_quantizer.computePartialEncoding(
            updated_encoding.bw,
            updated_encoding,
