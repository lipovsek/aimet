...
from aimet_torch.arch_checker.arch_checker_rules import (
    NODE_CHECK_DICT,
    PATTERN_CHECK_LIST,
)
...
class OpStructure(Op):
    ...
    def __init__(self, op_tuple: str):
        ...
        name = "_".join(
            [
                report_const.OP_STRUCT_OP_TYPE,
                first_op.name.split("_")[-1],
                last_op.name.split("_")[-1],
            ]
        )
    ...
    def get_module(self):
        ...
        return [
            op.get_module() if isinstance(op, Op) else OpStructure(op).get_module()
            for op in self.op_tuple
        ]
...
class NodeErrorReportObject:
    ...
    def get_issue_recomm_list(self) -> List[Tuple[str, str]]:
        ...
        return [
            _get_report_issue_recomm(_failed_check, self.op_type)
            for _failed_check in self.failed_checks
        ]
...
class ArchCheckerReport:
    ...
    def update_raw_report(
        self,
        op: Union[Op, List[Op], List[OpStructure]],
        failed_check: Union[Set[str], str],
    ):
        """
        ...
        """
        _update_raw_report(
            self._raw_report, _generate_arch_checker_report(op, failed_check)
        )
...
def _generate_arch_checker_report(
    op: Union[Op, List[Op], List[OpStructure]], failed_check: Union[Set[str], str]
) -> Dict[str, NodeErrorReportObject]:
    """
    ...
    return {
        _op.dotted_name_op: NodeErrorReportObject(_op, {failed_check}) for _op in op
    }
