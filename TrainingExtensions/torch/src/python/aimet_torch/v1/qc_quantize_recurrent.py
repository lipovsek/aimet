...
class QcQuantizeRecurrent(torch.nn.Module):  # pylint: disable=too-many-public-methods
    ...
    def __init__(
        self,
        module_to_quantize: Union[torch.nn.RNN, torch.nn.LSTM, torch.nn.GRU],
        weight_bw: int,
        activation_bw: int,
        round_mode: str,
        ...
        self._output_quantizers = self._create_activation_quantizers(
            outputs,
            activation_bw,
            self._round_mode,
            quant_scheme,
        ...
        inputs = (
            ["input_l{}", "initial_h_l{}", "initial_c_l{}"]
            if hasCellState
            else ["input_l{}", "initial_h_l{}"]
        )
        self._input_quantizers = self._create_activation_quantizers(
            inputs,
            activation_bw,
            self._round_mode,
            quant_scheme,
        ...
        self._create_param_quantizers(
            weight_bw, self._round_mode, quant_scheme, is_symmetric, data_type
        )
        ...
        # to be sub-optimal.
        self._reorder_initial_h_c_stats_update = (
            QcQuantizeRecurrent._is_initial_h_c_stats_update_reordered()
        )
    ...
    def _is_initial_h_c_stats_update_reordered() -> bool:
        ...
        for tensor_names in grouped_quantizer_map.values():
            if (
                "h_l{}" in tensor_names
                and "initial_h_l{}" in tensor_names
                or "c_l{}" in tensor_names
                and "initial_c_l{}" in tensor_names
            ):
                return True
        return False
    ...
    @staticmethod
    def _get_group_name(
        tensor_name: str, layer: Union[int, None] = None
    ) -> Union[str, None]:
        """
    ...
    def _create_activation_quantizers(
        self,
            ...
            for name in tensor_names:
                ...
                if group_name:
                    if group_name not in self._grouped_quantizers:
                        self._grouped_quantizers[group_name] = tensor_quantizer_factory(
                            activation_bw,
                            round_mode,
                            quant_scheme,
        ...
        for layer in range(self.num_layers):
            for group_name, tensor_names in grouped_quantizer_map.items():
                ...
                if name not in self._grouped_quantizers:
                    self._grouped_quantizers[name] = tensor_quantizer_factory(
                        weight_bw,
                        round_mode,
                        quant_scheme,
                        use_symmetric_encodings=is_symmetric,
                ...
                tensor_names = [
                    tensor_name.format(layer) for tensor_name in tensor_names
                ]
    ...
    def _set_default_eai_quantizer_state(self):
        ...
        default_group_quantizer_state = {
            name.format(layer): state
            for layer in range(self.num_layers)
            for name, state in default_group_quantizer_state.items()
        }

        tensor_names_with_grouped_quantizers = [
            name.format(layer)
            for layer in range(self.num_layers)
            for tensor_names in grouped_quantizer_map.values()
            for name in tensor_names
    ...
    def _clone_module_params(
        self, module: Union[torch.nn.RNN, torch.nn.LSTM, torch.nn.GRU]
    ):
        """
            ...
            for direction in range(self.num_directions):
                for param_name in self._get_param_names(direction, layer):
                    setattr(
                        self,
                        param_name,
                        torch.nn.Parameter(getattr(module, param_name).data.clone()),
                    )
            ...
            for param_quantizer, param_list in quantizer_param_map.items():
                ...
                for name, param in param_list:
                    quantized_params[name] = param.data = (
                        self._param_quantize_dequantize(param.clone(), param_quantizer)
                    )
        else:
            ...
            for name, _ in self.get_named_parameters():
                ...
                if quantizer.enabled:
                    ...
                    if quantizer in self._grouped_quantizers.values():
                        name, *_ = [
                            n
                            for n, q in self._grouped_quantizers.items()
                            if q == quantizer
                        ]
                    encoding_list_for_params.append(
    ...
    def _param_quantize_dequantize(
        self, data: torch.Tensor, param_quantizer: StaticGridPerTensorQuantizer
    ) -> torch.Tensor:
        """
    ...
    def construct_and_initialize_trainable_quantizers(self, quant_scheme):
        ...
        def _create_trainable_quantizer(bw, name, quantizer):
            ...
            # Input quantizer
            new_quantizer = tensor_quantizer_factory(
                bw,
                self._round_mode,
                quant_scheme,
                self._is_symmetric,
        ...
        new_grouped_quantizers = {
            name: _create_trainable_quantizer(self._activation_bw, name, quantizer)
            for name, quantizer in self._grouped_quantizers.items()
        }
        ...
        def create_trainable_quantizer(bw, name, quantizer):
            ...
            if quantizer in self._grouped_quantizers.values():
                group_names = [
                    n for n, q in self._grouped_quantizers.items() if q == quantizer
                ]
                assert len(group_names) == 1
                # creating a param min/max references to the shared group min/max parameters.
                setattr(
                    self,
                    f"{name}_encoding_min",
                    getattr(self, f"{group_names[0]}_encoding_min"),
                )
                setattr(
                    self,
                    f"{name}_encoding_max",
                    getattr(self, f"{group_names[0]}_encoding_max"),
                )
        ...
        self._input_quantizers = {
            name: create_trainable_quantizer(self._activation_bw, name, quantizer)
        ...
        self._param_quantizers = {
            name: create_trainable_quantizer(self._weight_bw, name, quantizer)
            for name, quantizer in self._param_quantizers.items()
        }
    ...
    def get_activation_param_quantizers_for_onnx_tensors(
        ...
        Dict[str, StaticGridPerTensorQuantizer], Dict[str, StaticGridPerTensorQuantizer]
    ]:
        """
        ...
        self._fill_quantizer_maps_for_inputs_and_params(
            activations_quantizer_map, params_quantizer_map, io_tensor_map
        )
    ...
    def _fill_quantizer_maps_for_inputs_and_params(
        self,
        activations_quantizer_map: Dict[str, StaticGridPerTensorQuantizer],
    ...
    def _fill_quantizer_maps_for_outputs(
        self,
        activations_quantizer_map: Dict[str, StaticGridPerTensorQuantizer],
        io_tensor_map: List[OpToIOTensors],
    ):
    ...
    def _quantize_activation(
        self,
        tensor_quantizer: Union[
            StaticGridPerTensorQuantizer, LearnedGridTensorQuantizer
        ],
        ...
        for input_tensor in tensors_to_quantize:
            if self._mode is QcQuantizeOpMode.ANALYSIS:
                ...
                output = tensor_quantizer.quantize_dequantize(
    ...
    rnn_impl_map = {
        "RNN_TANH": torch._VF.rnn_tanh_cell,
    ...
    @staticmethod
    def _format_hx_output(
        stacked_hx: Union[List[Tuple[torch.Tensor]], List[torch.Tensor]],
    ...
    def forward(
        self, inputs: Union[torch.Tensor, PackedSequence], hx: torch.Tensor = None
        ...
        inputs, packed_sequence_info = _get_inputs_and_packed_sequence_info(
            inputs, self.batch_first
        )
        ...
        with self._quantize_dequantize_params(inputs) as (quantized_params, _inputs):
            for layer in range(self.num_layers):
                # Quantize the inputs
                quantized_input = self._quantize_activation(
                    self._input_quantizers["input_l{}".format(layer)], _inputs
                )
                ...
                for direction in range(self.num_directions):
                    permutation = (
                        ...
                        else packed_sequence_info.unsorted_indices
                    )
                    update_initial_hx_encoding_stats, initial_hx = (
                        self._intialize_quantize_hidden_state(
                            batches, _inputs, layer, hx, permutation=permutation
                        )
                    )
                    cell_hx = initial_hx
                    ...
                    param = [
                        quantized_params[p]
                        for p in self._get_param_names(direction, layer)
                    ]
                    weight_ih, weight_hh, *bias = param
                    bias_ih, bias_hh = bias if bias else (None, None)

                    if direction == 1:
                        quantized_input = _get_flipped_input_for_reverse_pass(
                            quantized_input, packed_sequence_info, steps
                        )

                    for iteration in range(steps):
                        new_cell_hx = self.rnn_impl_map[self.mode](
                            quantized_input[iteration],
                            cell_hx,
                            weight_ih,
                            weight_hh,
                            bias_ih,
                        ...
                        cell_hx = _replace_appropriate_hidden_state_rows(
                            cell_hx,
                            new_cell_hx,
                        ...
                        if direction == 0:
                            output.append(
                                cell_hx[0] if isinstance(cell_hx, tuple) else cell_hx
                            )
                        else:
                            if not reverse_pass_output:
                                reverse_pass_output = [None] * (steps * batches)
                            _fill_appropriate_rows_in_reverse_pass_output(
                                reverse_pass_output,
                                packed_sequence_info,
                                steps,
                                batches,
                    ...
                    if update_initial_hx_encoding_stats:
                        self._update_encoding_stats_with_initial_hidden_state(
                            initial_hx, layer
                        )

                    if reverse_pass_output:
                        _concatenate_output_with_reverse_pass_output(
                            output,
                            reverse_pass_output,
                            self.hidden_size,
                            steps,
            ...
            output, stacked_hx = _reformat_output_and_stacked_hx_for_packed_sequence(
                output, stacked_hx, self.batch_first, packed_sequence_info
            )
            hx = QcQuantizeRecurrent._format_hx_output(stacked_hx)

        return output, hx

    def _quantize_hidden_cell_state(
        self, layer_index: int, cell_hx: Union[torch.Tensor, Tuple[torch.Tensor]]
        ...
        if isinstance(cell_hx, tuple):
            quantized_cell_hx = (
                self._quantize_activation(
                    self._output_quantizers["h_l{}".format(layer_index)], cell_hx[0]
                ),
                self._quantize_activation(
                    self._output_quantizers["c_l{}".format(layer_index)], cell_hx[1]
                ),
            )
        else:
            quantized_cell_hx = self._quantize_activation(
                self._output_quantizers["h_l{}".format(layer_index)], cell_hx
            )
        return quantized_cell_hx

    def _intialize_quantize_hidden_state(
        self,
        batches: int,
        inputs: torch.Tensor,
        layer: int,
        ...
        is_quantized = (
            self._mode != QcQuantizeOpMode.ANALYSIS
            or not self._reorder_initial_h_c_stats_update
        )
        if hx is None:
            zeros = torch.zeros(
                batches, self.hidden_size, dtype=inputs.dtype, device=inputs.device
            )
            ...
            if isinstance(hx, tuple):
                ...
                if is_quantized:
                    h = (
                        self._quantize_activation(
                            self._input_quantizers["initial_h_l{}".format(layer)], h[0]
                        ),
                        self._quantize_activation(
                            self._input_quantizers["initial_c_l{}".format(layer)], h[1]
                        ),
                    )
            else:
                ...
                h = hx[layer]
                if is_quantized:
                    h = self._quantize_activation(
                        self._input_quantizers["initial_h_l{}".format(layer)], h
                    )
        if permutation is not None:
            if isinstance(h, tuple):
                h = (
                    h[0].index_select(0, permutation),
                    h[1].index_select(0, permutation),
                )
            else:
    ...
    def _update_encoding_stats_with_initial_hidden_state(
        self,
        ...
        if isinstance(cell_hx, tuple):
            self._input_quantizers[
                "initial_h_l{}".format(layer_index)
            ].update_encoding_stats(cell_hx[0])
            self._input_quantizers[
                "initial_c_l{}".format(layer_index)
            ].update_encoding_stats(cell_hx[1])
        else:
            self._input_quantizers[
                "initial_h_l{}".format(layer_index)
            ].update_encoding_stats(cell_hx)


def _get_inputs_and_packed_sequence_info(
    inputs: Union[torch.Tensor, PackedSequence], batch_first: bool
    ...
    if isinstance(inputs, PackedSequence):
        ...
        packed_sequence_info = PackedSequenceInfo(
            sequence_lens, batch_sizes, unsorted_indices, sorted_indices
        )
...
def _get_flipped_input_for_reverse_pass(
    inputs: torch.Tensor, packed_sequence_info: PackedSequenceInfo, steps: int
) -> torch.Tensor:
    """
    ...
    if packed_sequence_info:
        ...
        for i, sequence_length in enumerate(sorted_lens):
            if sequence_length < steps:
                inputs_copy[:, i][:sequence_length] = inputs[:, i][
                    steps - sequence_length :
                ]
    return inputs_copy


def _replace_appropriate_hidden_state_rows(
    hidden_state: Union[Tuple[torch.Tensor, torch.Tensor]],
        ...
        if current_batch_size == num_batches:
            ...
            if isinstance(hidden_state, tuple):
                hidden_state = (
                    torch.cat(
                        (
                            new_hidden_state[0][: current_batch_size - num_batches],
                            hidden_state[0][current_batch_size - num_batches :],
                        )
                    ),
                    torch.cat(
                        (
                            new_hidden_state[1][: current_batch_size - num_batches],
                ...
                hidden_state = torch.cat(
                    (
                        new_hidden_state[: current_batch_size - num_batches],
...
def _fill_appropriate_rows_in_reverse_pass_output(
    reverse_pass_output,
    packed_sequence_info,
    steps,
    num_batches,
    ...
    for i in range(num_batches):
        ...
        reverse_pass_output_index = (
            num_batches * (steps - 1)
            - num_batches * iteration
            - num_batches * (steps - sequence_length)
            + i
        )
        output_to_append = (
            hidden_state[0][i] if isinstance(hidden_state, tuple) else hidden_state[i]
        )
        if reverse_pass_output_index >= 0:
            reverse_pass_output[reverse_pass_output_index] = output_to_append


def _concatenate_output_with_reverse_pass_output(
    output, reverse_pass_output, hidden_size, steps, num_batches, device
):
    """
    ...
    for i in range(steps):
        reverse_pass_batches.append(
            torch.stack(
                reverse_pass_output[(i * num_batches) : ((i + 1) * num_batches)]
            )
        )
...
def _reformat_output_and_stacked_hx_for_packed_sequence(
    output: torch.Tensor,
    stacked_hx: List[Union[Tuple[torch.Tensor, torch.Tensor], torch.Tensor]],
    batch_first: bool,
    packed_sequence_info: PackedSequenceInfo,
) -> Tuple[
    Union[torch.Tensor, PackedSequence],
    List[Union[Tuple[torch.Tensor, torch.Tensor], torch.Tensor]],
]:
    """
    ...
    if packed_sequence_info.sorted_indices is not None:
        output = output.index_select(
            0 if batch_first else 1, packed_sequence_info.sorted_indices
        )
        for i, hx_tensor in enumerate(stacked_hx):
            if isinstance(hx_tensor, tuple):
                stacked_hx[i] = (
                    hx_tensor[0].index_select(0, packed_sequence_info.sorted_indices),
                    hx_tensor[1].index_select(0, packed_sequence_info.sorted_indices),
                )
            else:
                stacked_hx[i] = hx_tensor.index_select(
                    0, packed_sequence_info.sorted_indices
                )
