...
onnx_subgraph_op_to_pytorch_module_param_name_index_based = {
    ...
    torch.nn.PReLU: {(0, "PRelu"): {1: "weight"}},
}
...
class CustomMarkerFunc(torch.autograd.Function):
    ...
    def forward(ctx, inp, _identifier, _start, _is_leaf):  # pylint: disable=arguments-differ, unused-argument
        return (
            inp.clone().detach()
        )  # clone prevents export tracing to avoid optimizing out the operation.
...
class CustomMarker(torch.nn.Module):
    ...
    def _apply_markers_to_tuple(
        self, inputs, is_start_marker, marked_tensor_map
        ...
        for t in inputs:
            if (
                id(t) in marked_tensor_map
            ):  # if tensor is already seen before map to the previous tensor
                t = marked_tensor_map[id(t)]
            else:
                key = id(t)
                if isinstance(t, torch.Tensor):
                    t = CustomMarkerFunc.apply(
                        t, self.identifier, is_start_marker, self.is_leaf
                    )
                elif isinstance(t, dict):
                    t = self._apply_marker_to_dict(
                        t, is_start_marker, marked_tensor_map
                    )
    ...
    def _apply_marker_to_dict(
        self, tensors_dict: Dict, is_start_marker: str, marked_tensor_map
    ) -> Dict:
        """
...
class OnnxSaver:
    ...
    @classmethod
    def create_onnx_model_with_pytorch_layer_names(
        cls,
        onnx_model_path: str,
        pytorch_model: torch.nn.Module,
        dummy_input: Union[torch.Tensor, Tuple, List],
        is_conditional=False,
        module_marker_map=None,
        ...
        for dropout_type in aimet_torch.utils.DROPOUT_TYPES:
            aimet_torch.utils.replace_modules(
                pytorch_model,
    ...
    def set_node_names(
        ...
        onnx_model = cls._map_onnx_nodes_to_pytorch_modules(
            pytorch_model,
            dummy_input,
            onnx_model_path,
            onnx_export_args,
    ...
    @classmethod
    def check_onnx_node_names(
        cls, onnx_model: onnx.ModelProto, pytorch_model: torch.nn.Module
    ):
        """
        ...
        root_module_names = tuple(
            local_module_name for local_module_name, _ in pytorch_model.named_children()
        )
        node_names = [
            node.name
            for node in onnx_model.graph.node
            if not node.name.startswith("Constant")
        ]
        num_nodes = len(node_names)
        node_names = set(node_names)
        if num_nodes != len(node_names):
            _logger.warning(
                "%d nodes do not have unique names", num_nodes - len(node_names)
            )

        num_named_nodes = sum(name.startswith(root_module_names) for name in node_names)
        _logger.info(
            "successfully created onnx model with %d/%d node names updated",
            num_named_nodes,
            num_nodes,
        )

    @staticmethod
    def _create_map_of_tensor_to_node(
        ...
        for node in onnx_model.graph.node:
            OnnxSaver._populate_input_output_tensor_maps(
                map_input_tensor_to_node, map_output_tensor_to_node, node
            )
        return map_output_tensor_to_node, map_input_tensor_to_node

    @staticmethod
    def _populate_input_output_tensor_maps(
        ...
        for output in node.output:
            assert output not in map_output_tensor_to_node, (
                "More than one node produces the same tensor"
            )
        ...
        for attribute in node.attribute:
            if getattr(attribute, "g").name != "":
                for subnode in getattr(attribute, "g").node:
                    OnnxSaver._populate_input_output_tensor_maps(
                        map_input_tensor_to_node, map_output_tensor_to_node, subnode
                    )
            ...
            # recursively call children modules
                ...
                if add_all_marker and not isinstance(module_ref, torch.nn.ModuleList):
                    if not isinstance(module_ref, CustomMarker):
                        full_module_name = module_name_map[module_ref]
                        marker_layer = CustomMarker(
                            module_ref, full_module_name, "False"
                        )
                        setattr(starting_module, local_module_name, marker_layer)
                    else:
                        ...
                        if module_name != module_ref.identifier:
                            _logger.warning(
                                "layer=%s already marked as '%s', skipping",
                                module_name,
    ...
    @classmethod
    def _map_onnx_nodes_to_pytorch_modules(
        cls,
        pt_model,
        dummy_input,
        onnx_model_path,
        ...
        onnx_model, onnx_model_all_marker = cls._create_onnx_model(
            dummy_input,
            is_conditional,
            module_marker_map,
            onnx_export_args,
        ...
        graphs_list, output_names_list = OnnxSaver._get_graph_and_output_names_lists(
            onnx_model
        )
        ...
        map_output_tensor_to_node, map_input_tensor_to_node = (
            cls._create_map_of_tensor_to_node(onnx_model)
        )
        ...
        # Remove markers
        cls._detach_start_and_end_markers(
            map_input_tensor_to_node,
            map_output_tensor_to_node,
            start_marker_map,
            end_marker_map,
    ...
    def _create_onnx_model(
    ...
    ) -> Tuple[onnx.NodeProto, Optional[onnx.NodeProto]]:
        """
        ...
        """
        try:
            onnx_model = cls._create_onnx_model_with_markers(
                dummy_input,
                pt_model,
                working_dir,
                onnx_export_args,
    ...
    @classmethod
    def _update_non_leaf_pytorch_modules_onnx_nodes_names(
        cls,
        pt_model: torch.nn.Module,
        dummy_input,
        working_dir: str,
        ...
        try:
            ...
            cls._update_non_leaf_onnx_nodes_names(
                node_list_from_leaf_markers=cls._get_topological_sorted_nodes_list(
                    onnx_model
                ),
                node_list_from_all_markers=cls._get_topological_sorted_nodes_list(
                    onnx_model_all_marker
                ),
            ...
            _logger.error(
                "failed with exception when naming of onnx op at non-leaf modules",
                exc_info=True,
            )
            _logger.warning(
                "naming of onnx op at non-leaf modules failed, skipping naming of non-leaf"
            )

    @classmethod
    def _update_non_leaf_onnx_nodes_names(
        cls,
        ...
        """
        i = 0
        for (node, pt_module_name), (leaf_only_node, _) in zip(
            node_list_from_all_markers, node_list_from_leaf_markers
        ):
            if node.op_type != leaf_only_node.op_type:
                _logger.warning(
                    "leaf (%s:%s) and non-leaf (%s:%s) sequence did not match at %d",
                    node.name,
                    node.op_type,
                    leaf_only_node.name,
                    leaf_only_node.op_type,
                    i,
                )
                break

            if (
                pt_module_name is not None
                and "#" not in leaf_only_node.name
                and leaf_only_node.name != pt_module_name
            ):
        ...
        if i < len(node_list_from_leaf_markers) - 1:
            _logger.warning(
                "partially (%d/%d) named onnx op at non-leaf modules",
                i,
                len(node_list_from_leaf_markers),
            )
            cls.save_mismatch_sequence(
                i, node_list_from_all_markers, node_list_from_leaf_markers, working_dir
            )

    @classmethod
    def save_mismatch_sequence(
        cls,
        mismatch_index: int,
        node_list_from_leaf_markers: List[Tuple[onnx.NodeProto, str]],
        node_list_from_all_markers: List[Tuple[onnx.NodeProto, str]],
        ...
        """
        sequence = []
        for index, ((node, pt_module_name), (leaf_only_node, _)) in enumerate(
            zip(node_list_from_all_markers, node_list_from_leaf_markers)
        ):
            sequence.append(
                {
                    "index": index,
                    "leaf": {
                        "name": leaf_only_node.name,
                        "op_type": leaf_only_node.op_type,
                    },
                    "non-leaf": {
                        "name": node.name,
                        "op_type": node.op_type,
                        "module_name": pt_module_name,
                    },
                }
            )
        ...
        with open(filename, "w") as f:
            yaml.dump(
                {"mismatch": mismatch_index, "sequence": sequence},
                f,
    ...
    @classmethod
    def _get_topological_sorted_nodes_list(
        cls, onnx_model
    ) -> List[Tuple[onnx.NodeProto, Optional[str]]]:
        """
                ...
                if node.op_type == "CustomMarker":
                    ...
                    if is_start_marker == "True":
                        ...
                        if identifier in marker_context:
                            ...
                            _logger.warning(
                                "end-marker seen without passing start-marker for '%s', continue to "
                                "use parent context '%s'",
                                identifier,
                                parent_context,
                            )
                else:
                ...
                for tensor in node.output:
                    ...
                    for dnode in reversed(downstream_nodes):
                        ...
                        skip = any(
                            ...
                            and map_output_tensor_to_node[input_tensor].op_type
                            not in ["Constant"]
                            and id(map_output_tensor_to_node[input_tensor])
                            not in visited
    ...
    @staticmethod
    def _get_graph_and_output_names_lists(
        onnx_model: onnx.ModelProto,
    ) -> Tuple[List[onnx.GraphProto], List[List[str]]]:
        """
        ...
        graphs_list = []
        output_names_list = []
        OnnxSaver._populate_graph_and_output_names_lists(
            onnx_model.graph, graphs_list, output_names_list
        )
    ...
    @staticmethod
    def _populate_graph_and_output_names_lists(
        onnx_graph: onnx.GraphProto,
        graphs_list: List[onnx.GraphProto],
        ...
        for node in onnx_graph.node:
            for attribute in node.attribute:
                if getattr(attribute, "g").name != "":
                    OnnxSaver._populate_graph_and_output_names_lists(
                        attribute.g, graphs_list, output_names_list
                    )

    @staticmethod
    def _get_onnx_node_map(
        onnx_graph: onnx.GraphProto,
        onnx_node_map: Dict[Tuple[str, str], onnx.NodeProto] = None,
    ...
    @classmethod
    def _fix_initializer_names(
        cls, onnx_model: onnx.NodeProto, pt_model: torch.nn.Module
    ):
        """
        ...
        initializer_name_to_index = {
            initializer.name: idx for idx, initializer in enumerate(initializers)
        }
        ...
        for module_name, module_ref in pt_model.named_modules():
            ...
            if type(module_ref) in onnx_subgraph_op_to_pytorch_module_param_name:
                for (
                    node_suffix,
                    op_type,
                ), replace_pairs in onnx_subgraph_op_to_pytorch_module_param_name[
                    type(module_ref)
                ].items():
    ...
    @classmethod
    def _fix_initializer_names_for_export_to_onnx_direct(
        cls, onnx_model: onnx.NodeProto, pt_model: torch.nn.Module
    ):
        """
        ...
        for module_name, module_ref in pt_model.named_modules():
            if type(module_ref) in onnx_subgraph_op_to_pytorch_module_param_name:
                if module_name not in pt_name_to_onnx_nodes:
                    error_str = (
                        f"{module_name} of type {module_ref} not found in pt_name_to_onnx_nodes dictionary."
                    ...
                    for (
                        (idx, op_type),
                        replace_pairs,
                    ) in onnx_subgraph_op_to_pytorch_module_param_name_index_based[
                        type(module_ref)
                    ].items():
    ...
    @classmethod
    def _replace_param_name(
        cls,
        initializers: List[onnx.TensorProto],
        initializer_name_to_index: Dict[str, int],
        module_name: str,
        node: onnx.NodeProto,
        replace_pairs: Dict[int, str],
    ):
        """
            ...
            if len(node.input) > input_index:
                ...
                if "." in old_param_name:
                    if old_param_name != new_param_name:
                        print(
                            f"{old_param_name} != {new_param_name} expected param name set"
                        )
                    continue
    ...
    def _remove_marked_module_string_from_node_inp_out_names(
        ...
        for node in onnx_graph.node:
            ...
            for index, param_name in enumerate(node.input):
                updated_name = cls._get_updated_name(param_name)
                if MAKE_NODE_OUTPUT_NAME_UNIQUE:
                    updated_name = param_name_to_updated_name.get(
                        param_name, updated_name
                    )
    ...
    def _remove_detached_nodes_from_onnx_graph(cls, onnx_graph: onnx.GraphProto):
        ...
        """
        marker_nodes = [
            node for node in onnx_graph.node if node.op_type == "CustomMarker"
        ]
    ...
    def _set_onnx_node_names(cls, map_input_tensor_to_node, start_marker_map):
        ...
        node_name_count_map = {}
        visited = set()
        leaf_only_start_marker = {
            n: m
            for n, m in start_marker_map.items()
        ...
        def set_name_for_downstream_nodes(starting_nodes, name, depth):
            for node in starting_nodes:
                ...
                if node.name in node_name_count_map:
                    ...
                    node.name = (
                        f"{node.name}-{reuse_count}"
                        if "#" in node.name
                        else f"{node.name}#0-{reuse_count}"
                    )
                else:
    ...
    def _create_map_of_marker_nodes(cls, onnx_model):
        ...
        for node in onnx_model.graph.node:
            OnnxSaver._populate_start_and_end_marker_maps(
                start_marker_map, end_marker_map, node
            )
        return end_marker_map, start_marker_map

    @staticmethod
    def _populate_start_and_end_marker_maps(
        start_marker_map: Dict[str, onnx.NodeProto],
        end_marker_map: Dict[str, onnx.NodeProto],
        node: onnx.NodeProto,
    ):
        """
        ...
        for attribute in node.attribute:
            if getattr(attribute, "g").name != "":
                for subnode in getattr(attribute, "g").node:
                    OnnxSaver._populate_start_and_end_marker_maps(
                        start_marker_map, end_marker_map, subnode
                    )

    @classmethod
    # pylint: disable=too-many-locals
    def _create_onnx_model_with_markers(
        cls,
        dummy_input,
        pt_model,
        working_dir,
        ...
        cls._add_markers(
            model, module_name_map, module_marker_map, is_conditional, add_all_markers
        )
        temp_file = os.path.join(
            working_dir,
            "temp_onnx_model_with_markers.onnx"
            if not add_all_markers
            else "temp_onnx_model_with_all_markers.onnx",
        )

        cls._export_model_to_onnx(
            model, dummy_input, temp_file, is_conditional, onnx_export_args
        )
    ...
    @classmethod
    def _detach_start_and_end_markers(
        cls,
        ...
        for markers in start_marker_map.values():
            for marker in markers:
                cls._detach_start_marker_node(
                    map_input_tensor_to_node, map_output_tensor_to_node, marker
                )

        output_names_list = copy.deepcopy(output_names_list)
        for markers in end_marker_map.values():
            for marker in markers:
                cls._detach_end_marker_node(
                    map_input_tensor_to_node,
                    map_output_tensor_to_node,
                    graphs_list,
                    output_names_list,
    ...
    def _detach_start_marker_node(
        ...
        del map_output_tensor_to_node[
            output_tensor
        ]  # No node should produce output tensor anymore
        del map_input_tensor_to_node[
            output_tensor
        ]  # No node should consume output tensor anymore

        start_marker.input.pop()
        start_marker.output.pop()

    @classmethod
    def _detach_end_marker_node(
        cls,
        map_input_tensor_to_node: Dict[str, onnx.NodeProto],
        map_output_tensor_to_node: Dict[str, List[onnx.NodeProto]],
        graphs_list: List[onnx.GraphProto],
    ...
    def _set_output_names(
        ...
        OnnxSaver._set_output_names_for_graph(
            onnx_model.graph,
            graphs_list,
            output_names_for_all_graphs,
            map_output_tensor_to_node,
            map_input_tensor_to_node,
        )
    ...
    @staticmethod
    def _set_output_names_for_graph(
        ...
        for index, output in enumerate(onnx_graph.output):
            ...
            if old_tensor in map_input_tensor_to_node:
                ...
                del map_input_tensor_to_node[
                    old_tensor
                ]  # No node should consume old tensor anymore
    ...
    @staticmethod
    def _get_all_nodes(
        onnx_graph: onnx.GraphProto, all_nodes: Union[List[onnx.NodeProto], None] = None
    ) -> List[onnx.NodeProto]:
        """
    ...
    @staticmethod
    def _collate_io_tensors_for_multi_layer_recurrent_nodes(
        onnx_model: onnx.NodeProto, node_to_io_tensor_name_map: Dict
    ):
        """
        ...
        for root_node in root_nodes.values():
            ...
            other_layers = [
                ...
                if node.startswith(root_node.split("#")[0])
            ]
    ...
    @staticmethod
    def _get_all_initializers(
        onnx_graph: onnx.GraphProto,
        initializers: Union[List[onnx.TensorProto], None] = None,
    ...
    def _populate_node_to_io_tensor_map_and_valid_param_set(
        ...
        for node in onnx_graph.node:
            if node.name:
                onnx_node_io_tensors = OpToIOTensors(
                    list(node.input), list(node.output)
                )
                if (
                    node.name not in node_to_io_tensor_name_map
                ) or node.op_type in recurrent_onnx_optypes:
            ...
            for attribute in node.attribute:
                if getattr(attribute, "g").name != "":
                    OnnxSaver._populate_node_to_io_tensor_map_and_valid_param_set(
                        attribute.g,
                        initializer_names,
    ...
    @classmethod
    def get_onnx_node_to_io_tensor_names_map(
        cls, onnx_model: onnx.NodeProto
        ...
        valid_param_set = set()
        initializer_names = [
            ini.name for ini in OnnxSaver._get_all_initializers(onnx_model.graph)
        ]
        OnnxSaver._populate_node_to_io_tensor_map_and_valid_param_set(
            onnx_model.graph,
            initializer_names,
        ...
        if (
            ...
            or not EXPORT_TO_ONNX_DIRECT
        ):
            cls._collate_io_tensors_for_multi_layer_recurrent_nodes(
                onnx_model, node_to_io_tensor_name_map
            )
    ...
    def _export_model_to_onnx(
        ...
        # Set dynamo=False for torch 2.9.0 and above.
        # The default value of dynamo is True from torch 2.9.0
        ...
        if version.parse(torch.__version__) < version.parse("1.11.0"):
            ...
            try:
                ...
                if isinstance(e, CheckerError) and os.path.isfile(temp_file):
                    _logger.warning(
                        "ONNX Checker has failed but ONNX graph is still generated."
                    )
                else:
                    raise e
...
def save_initializer_restored_onnx_graph(
    ...
    onnx.save(
        restored_model, restored_model_path, save_as_external_data=save_as_external_data
    )


def restore_onnx_graph_initializers(
...
def _get_pruned_initializer_map(
    onnx_graph: onnx.GraphProto,
    initializers: List[onnx.TensorProto],
    initializer_names: List[str],
) -> Dict[str, PrunedInitializerInfo]:
    """
...
def _restore_pruned_initializer(
    onnx_graph: onnx.GraphProto,
    input_tensor: str,
...
__deleted_atttributes__ = {"simplify_onnx_model": "aimet_torch==2.0"}
