...
from ..connected_graph.operation import (
    Op,
...
from ..utils import AimetLogger, ModelApi, api_channel_index_dict
...
class MaskPropagator:
    ...
    def _propagate_inter_module_masks(self):
        ...
        for a_product in self._products.values():
            ...
            if inter_module and a_product.producer in self._op_to_mask_dict:
                ...
                if isinstance(
                    self._op_to_mask_dict[producer].internal_connectivity,
                    StopInternalConnectivity,
                ):
                    continue
                ...
                for consumer in consumers:
                    if consumer in self._op_to_mask_dict:
                        consumer_connectivity = self._op_to_mask_dict[
                            consumer
                        ...
                        if isinstance(consumer_connectivity, StopInternalConnectivity):
                            continue
                        if isinstance(
                            consumer_connectivity, ConcatInternalConnectivity
                        ):
                            self._propagate_up_concat_inter_module_masks(
                                consumer, a_product
                            )
                        elif isinstance(consumer_connectivity, AddInternalConnectivity):
                            self._propagate_up_add_masks(consumer, a_product)
                        elif isinstance(
                            consumer_connectivity, SkipInternalConnectivity
                        ):
                            ...
                            self._set_inter_module_producer_output_and_consumer_input_mask(
                                consumer, a_product
                            )
        ...
        for op, op_mask in self._op_to_mask_dict.items():
            ...
            if ip_masks:
                for num in range(len(ip_masks)):
                    ip_mask_zero_positions = [
                        i for i in range(len(ip_masks[num])) if ip_masks[num][i] == 0
                    ]
            ...
            if op_masks:
                for num in range(len(op_masks)):
                    op_mask_zero_positions = [
                        i for i in range(len(op_masks[num])) if op_masks[num][i] == 0
                    ]
                    # TODO: remove 'Add', 'Concat' when old CG is gone
                    if (
                        op.type in ("Add", "Concat", "Split", "add", "cat", CG_SPLIT)
            ...
            if ip_mask_zero_positions_list or op_mask_zero_positions_list:
                logger.debug(
                    "Op: %s ip mask zero indices: %s, op mask zero indices: %s",
                    op.dotted_name,
                    ip_mask_zero_positions_list,
                    op_mask_zero_positions_list,
                )
    ...
    def get_ops_with_non_default_ip_op_masks(self) -> List[Op]:
        ...
        for op, op_mask in self._op_to_mask_dict.items():
            check_op = False
            if self._model_api == ModelApi.pytorch and op.type in (
                "Dropout",
                "Relu",
                "ReLU",
                "MaxPool",
    ...
    def _set_inter_module_producer_output_and_consumer_input_mask(
        self, consumer_op: Op, input_product: Product
    ):
        """
        ...
        if num_consumer_in_masks == 1:
            ...
            consumer_mask_index = (
                determine_preceding_op_input_product_index_in_multi_input_op(
                    producer, consumer_op
                )
            )
        else:
            logger.error(
                "Number of input masks for Op: %s is None", consumer_op.dotted_name
            )
        ...
        if num_producer_out_masks == 1:
            ...
            producer_mask_index = (
                determine_succeeding_op_output_product_index_in_multi_output_op(
                    consumer_op, producer
                )
            )
        else:
            logger.error(
                "Number of output masks for Product: %s is None", input_product.name
            )

        # Create the connection mask and set the Producer Op's output mask and the Consumer Op's input mask.
        connection_mask = (
            producer_out_masks[producer_mask_index]
            and consumer_in_masks[consumer_mask_index]
        )
        producer_mask.set_output_channel_mask(producer_mask_index, connection_mask)
        logger.debug(
            "Connection propagation: Op: %s, Product: %s, number of producer masks: %s, "
    ...
    def _propagate_up_concat_inter_module_masks(
        self, concat_op: Op, input_product: Product
    ):
        """
        ...
        logger.debug(
            "Propagate up concat: For Concat Op: %s, all input product names: %s",
            concat_op.dotted_name,
        ...
        logger.debug(
            "Propagate up concat: Processing input product: %s. Concat's input mask lengths: %s",
            input_product.name,
            [len(concat_mask) for concat_mask in concat_in_masks],
        )
        ...
        for concat_input_op_index, input_op in enumerate(concat_op.input_ops):
            logger.debug(
                "Propagate up concat: input Op: %s, Concat Op's index for this input op: %s, mask length: %s",
                input_op.dotted_name,
                concat_input_op_index,
                len(concat_in_masks[concat_input_op_index]),
            )

            if input_product.producer.dotted_name == input_op.dotted_name:
                logger.debug(
                    "Propagate up concat: Matching Product: %s with input_op: %s",
                    input_product.name,
                    input_op.dotted_name,
                )

                for product_consumer_index, consumer in enumerate(
                    input_product.consumers
                ):
                    if consumer.dotted_name == concat_op.dotted_name:
                        logger.debug(
                            "Propagate up concat: Input op's index for the Concat Op: %s",
                        ...
                        connection_mask = (
                            input_producer_out_masks[product_consumer_index]
                            and concat_in_masks[concat_input_op_index]
                        )

                        if (
                            input_product.producer.type in "Split"
                            or input_product.producer.type in CG_SPLIT
                        ):
                            logger.debug(
                                "Not propagating masks from Concat: %s to Split: %s",
                                concat_op.dotted_name,
                            ...
                            concat_op_mask.set_input_channel_mask(
                                concat_input_op_index, modified_mask
                            )

                        else:
                            input_op_mask.set_output_channel_mask(
                                product_consumer_index, connection_mask
                            )
                            concat_op_mask.set_input_channel_mask(
                                concat_input_op_index, connection_mask
                            )
                        # No need to check other consumers.
                        break

        logger.debug(
            "Propagate up concat: Completed processing input product: %s, input mask lengths: %s",
            input_product.name,
            [len(concat_mask) for concat_mask in concat_in_masks],
        )
    ...
    def _propagate_up_add_masks(self, add_op: Op, product: Product):
        ...
        """
        logger.debug(
            "propagate_up_add_masks: Add's inputs: %s",
            [product.name for product in add_op.inputs],
        )
        ...
        for index, a_product in enumerate(add_op.inputs):
            ...
            if (
                a_product.producer is not None
                and a_product.producer.dotted_name == product.producer.dotted_name
            ):
                if isinstance(
                    self._op_to_mask_dict[a_product.producer].internal_connectivity,
                    SplitInternalConnectivity,
                ):
                    add_op_mask = self._op_to_mask_dict[add_op]
                    logger.debug(
                        "Not propagating to Split. Restoring mask to default value."
                    )
    ...
    def _validate_and_adjust_split_op_masks(self, op: Op):
        ...
        for consumer in op.output_ops:
            while consumer in self._op_to_mask_dict and isinstance(
                self._op_to_mask_dict[consumer].internal_connectivity,
                DirectInternalConnectivity,
            ):
                ...
                logger.debug(
                    "Masks adjusted for: %s, %s", consumer.dotted_name, consumer.type
                )
    ...
    def _adjust_downstream_op_masks(
        ...
        if downstream_op.type not in get_conv_ops_for_api(model_api):
            ...
            if isinstance(
                self._op_to_mask_dict[downstream_op].internal_connectivity,
                SplitInternalConnectivity,
            ):
                ...
                for index in range(num_out_masks):
                    ...
                    self._adjust_downstream_op_masks(
                        downstream_op.output_ops[index], modified_mask, model_api
                    )
            elif not isinstance(
                self._op_to_mask_dict[downstream_op].internal_connectivity,
                StopInternalConnectivity,
                ...
                if downstream_op.output_ops:
                    self._adjust_downstream_op_masks(
                        downstream_op.output_ops[0], modified_mask, model_api
                    )
            else:
    ...
    def update_channels_to_winnow(
        ...
        name: str,
        reshape: bool,
        input_channels_to_winnow: Union[None, List[int]],
        ...
        if module_op:
            if reshape:
                ...
                self._op_to_mask_dict[module_op].update_channels_to_winnow(
                    input_channels_to_winnow, output_channels_to_winnow
                )
            else:
                ...
                if reshape_needed:
                    logger.debug(
                        "Reshape flag set to False. Module :%s will not be winnowed.",
            ...
            raise RuntimeError(
                "For the module, an Op was not found in the ConnectedGraph:", name
            )
