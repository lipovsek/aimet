...
from .bokeh_plots import DataTable
from .bokeh_plots import LinePlot
from .bokeh_plots import ProgressBar
from .utils import AimetLogger
from .curve_fit import MonotonicIncreasingCurveFit
from .defs import (
    CostMetric,
    LayerCompRatioPair,
    GreedyCompressionRatioSelectionStats,
    EvalFunction,
)
from .pruner import Pruner
from . import cost_calculator as cc
from .layer_database import Layer, LayerDatabase
from .comp_ratio_rounder import CompRatioRounder
...
class CompRatioSelectAlgo(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def select_per_layer_comp_ratios(
        self,
    ) -> Tuple[List[LayerCompRatioPair], Any]:
        """
...
class GreedyCompRatioSelectAlgo(CompRatioSelectAlgo):
    ...
    def __init__(
        ...
        num_candidates: int,
        use_monotonic_fit: bool,
        saved_eval_scores_dict: Optional[str],
        comp_ratio_rounding_algo: CompRatioRounder,
        use_cuda: bool,
        bokeh_session,
    ):
        # pylint: disable=too-many-arguments
        CompRatioSelectAlgo.__init__(
            self, layer_db, cost_calculator, cost_metric, comp_ratio_rounding_algo
        )
        ...
        if saved_eval_scores_dict:
            ...
            for index in range(1, num_candidates):
                self._comp_ratio_candidates.append(
                    (Decimal(1) / Decimal(num_candidates)) * index
                )
    ...
    def _pickle_eval_scores_dict(self, eval_scores_dict):
        ...
        logger.info(
            "Greedy selection: Saved eval dict to %s", self.PICKLE_FILE_EVAL_DICT
        )
    ...
    def _unpickle_eval_scores_dict(saved_eval_scores_dict_path: str):
        ...
        logger.info(
            "Greedy selection: Read eval dict from %s", saved_eval_scores_dict_path
        )
        return eval_dict

    @staticmethod
    def _calculate_function_value_by_interpolation(
        comp_ratio: Decimal, layer_eval_score_dict: dict, comp_ratio_list: List
    ):
        """
        ...
        if comp_ratio in comp_ratio_list:
            ...
            if ind == len(comp_ratio_list) - 1:
                ...
                eval_score = (float(comp_ratio) - float(x1)) * (y1 - y2) / (
                    float(x1) - float(x2)
                ) + y1
        return eval_score

    def _update_eval_dict_with_rounding(
        self, eval_scores_dict, rounding_algo, cost_metric
    ):
        updated_eval_dict = {}
        ...
        for layer_name in eval_scores_dict:
            ...
            for comp_ratio in layer_eval_dict:
                ...
                eval_score = self._calculate_function_value_by_interpolation(
                    rounded_comp_ratio, layer_eval_dict, comp_ratio_list
                )
    ...
    def _fit_eval_dict_to_monotonic_function(eval_scores_dict):
        for layer in eval_scores_dict:
            ...
            eval_scores, polynomial_coefficients = MonotonicIncreasingCurveFit.fit(
                comp_ratios, eval_scores
            )
            logger.debug(
                "The coefficients for layer %s are %s",
                layer,
                str(polynomial_coefficients),
            )
    ...
    def _construct_eval_dict(self):
        ...
        if self._saved_eval_scores_dict:
            eval_scores_dict = self._unpickle_eval_scores_dict(
                self._saved_eval_scores_dict
            )
    ...
    def select_per_layer_comp_ratios(self):
        ...
        updated_eval_scores_dict = self._update_eval_dict_with_rounding(
            eval_scores_dict, self._rounding_algo, self._cost_metric
        )

        # Get the overall min and max scores
        current_min_score, current_max_score = self._find_min_max_eval_scores(
            updated_eval_scores_dict
        )
        exit_threshold = (current_max_score - current_min_score) * 0.0001
        logger.info(
            "Greedy selection: overall_min_score=%f, overall_max_score=%f",
            current_min_score,
            current_max_score,
        )
        ...
        while True:
            ...
            current_comp_ratio = self._calculate_model_comp_ratio_for_given_eval_score(
                current_mid_score, updated_eval_scores_dict, original_model_cost
            )

            logger.debug(
                "Greedy selection: current candidate - comp_ratio=%f, score=%f, search-window=[%f:%f]",
                current_comp_ratio,
                current_mid_score,
                current_min_score,
                current_max_score,
            ...
            should_exit, selected_score = self._evaluate_exit_condition(
                current_min_score,
                current_max_score,
                exit_threshold,
                current_comp_ratio,
                self._target_comp_ratio,
            )
        ...
        layer_ratio_list = self._find_all_comp_ratios_given_eval_score(
            selected_score, updated_eval_scores_dict
        )
        selected_comp_ratio = self._calculate_model_comp_ratio_for_given_eval_score(
            selected_score, updated_eval_scores_dict, original_model_cost
        )

        logger.info(
            "Greedy selection: final choice - comp_ratio=%f, score=%f",
            selected_comp_ratio,
            selected_score,
        )

        return layer_ratio_list, GreedyCompressionRatioSelectionStats(
            updated_eval_scores_dict
        )

    @staticmethod
    def _evaluate_exit_condition(
        min_score, max_score, exit_threshold, current_comp_ratio, target_comp_ratio
    ):
    ...
    def _find_all_comp_ratios_given_eval_score(
        self, given_eval_score, eval_scores_dict
    ):
        layer_ratio_list = []
    ...
    @staticmethod
    def _find_layer_comp_ratio_given_eval_score(
        eval_scores_dict: Dict[str, Dict[Decimal, float]],
        given_eval_score,
        layer: Layer,
    ):
    ...
    def _compute_eval_scores_for_all_comp_ratio_candidates(
        self,
        ...
        if self.bokeh_session:
            ...
            progress_bar = ProgressBar(
                total=num_layers * num_candidates,
                title="Eval Scores Table",
                color="green",
        ...
        for layer in selected_layers:
            layer_wise_eval_scores = (
                self._compute_layerwise_eval_score_per_comp_ratio_candidate(
                    data_table, progress_bar, layer
                )
            )
    ...
    def _compute_layerwise_eval_score_per_comp_ratio_candidate(
        self, tabular_progress_object, progress_bar, layer: Layer
    ) -> Dict[Decimal, float]:
        """
        ...
        for comp_ratio in self._comp_ratio_candidates:
            logger.info(
                "Analyzing compression ratio: %s =====================>", comp_ratio
            )
            ...
            eval_score = self._eval_func(
                pruned_layer_db.model, self._eval_iter, use_cuda=self._is_cuda
            )
            ...
            logger.info(
                "Layer %s, comp_ratio %f ==> eval_score=%f",
                layer.name,
                comp_ratio,
                eval_score,
            )
            ...
            if self.bokeh_session:
                layer_wise_eval_scores_plot.update(
                    new_x_coordinate=comp_ratio, new_y_coordinate=eval_score
                )
                # Update the data table by adding the computed eval score
                tabular_progress_object.update_table(
                    str(comp_ratio), layer.name, eval_score
                )
...
class ManualCompRatioSelectAlgo(CompRatioSelectAlgo):
    ...
    def __init__(
        self,
        layer_db: LayerDatabase,
        layer_comp_ratio_pairs: List[LayerCompRatioPair],
        comp_ratio_rounding_algo: CompRatioRounder,
        cost_metric: CostMetric,
    ):
        CompRatioSelectAlgo.__init__(
    ...
    def select_per_layer_comp_ratios(self):
        for pair in self._layer_comp_ratio_pairs:
            rounded_comp_ratio = self._rounding_algo.round(
                pair.layer, pair.comp_ratio, self._cost_metric
            )
