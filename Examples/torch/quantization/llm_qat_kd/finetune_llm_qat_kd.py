...
def _evaluate(model, dataloader, num_batches):
    ...
    for batch_id, batch in enumerate(
        tqdm(dataloader, total=num_batches, desc="evaluate")
    ):
...
def train_kd(
    student_engine: DeepSpeedEngine,
...
def _train_kd(
    ...
    for _ in range(NUM_EPOCHS):
        for batch_id, batch in enumerate(
            tqdm(dataloader, total=num_batches, desc="train_kd")
        ):
            if batch_id >= num_batches:
                break
            ...
            with torch.no_grad():
                teacher_logits, *_ = teacher_engine(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                )
            ...
            with enable_recompute():
                student_logits, *_ = student_engine(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                )
            ...
            ce_loss = F.cross_entropy(
                student_logits.flatten(0, 1), labels.flatten(0, 1)
            )
...
def create_quant_sim(model, config_file, train_dataloader):
    ...
    with ds.zero.GatheredParameters(model.parameters()):
        ...
        for name, module in model.named_modules():
            ...
            if isinstance(module._module_to_wrap, torch.nn.LayerNorm) or name.endswith(
                "norm"
            ):
        ...
        sim.compute_encodings(
            lambda model, _: evaluate(
                model, train_dataloader, NUM_BATCHES_FOR_CALIBRATION
            ),
            None,
        )
    ...
    config.update(
        {
            ...
            // (TRAIN_MICRO_BATCH_SIZE_PER_GPU * dist.get_world_size()),
        }
    )
    return config
...
def main():
    ...
    sim = create_quant_sim(
        student, "default_per_channel_quantsim_config.json", train_dataloader
    )
    student_engine, optimizer, _, lr_scheduler = ds.initialize(
        model=sim.model, config=load_deepspeed_config_student()
    )

    teacher = get_teacher_model().to(LOCAL_RANK)
    teacher_engine = ds.init_inference(
        model=teacher, config=load_deepspeed_config_teacher()
    )
