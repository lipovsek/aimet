...
Usage:
    ...
    python -m GenAITests --framework torch --config GenAITests/example_config.yaml --online
    ...
    python -m GenAITests --framework onnx --config cfg.yaml --force-export -v
"""
...
def run_local(args, extra_pytest_args):
    ...
    for framework in frameworks:
        cmd = [
            ...
            "pytest",
            "-s",
            f"GenAITests/{framework}/test_genai.py",
            "--config",
            args.config,
            *forwarded,
            *extra_pytest_args,
...
def _print_summary(results_dir, config_path=None):
    ...
    if os.path.exists(results_json):
        from GenAITests.shared.helpers.summary import print_summary
...
def dispatch_online(args):
    ...
    print(
        f"  python -m GenAITests --framework {args.framework} --config {args.config} --download {run_id}"
    )
...
def _download_and_merge(gh, run_id, results_dir, frameworks):
    ...
    from GenAITests.shared.helpers.profiler import merge_csv_results, merge_json_results
